...
> **Current Focus:** Batch-2 UI sessions. Sessions 1–3 built (v0.55.0–v0.57.0): Delete Title with a trash folder (user-initiated deletion moves folders to `_trash/`, never a permanent delete; sync ignores `_trash/`; two-step confirmation in BookDetail), Merge now trashes the source folder — merge is a records operation, files never move between folders (decision reversed 2026-07-12), so sync can no longer re-ingest a merged-away title — and BookDetail's Files section now owns all format actions (v0.57.0): per-edition rows with filename + size, Add format / Add files, per-row remove that trashes the file with the same files-first contract. v0.53.0 (S15.3a + S15.3b + S15.2b) and v0.54.0 (P1 Overwrite Contract) deployed together 2026-07-12; the 618 damaged titles are closed — not restored (decided 2026-07-12), moving forward from current state. 10.0D Session 11 Final Audit remains open; 10.2+ pending go-decision. v0.58.0 (Session A — subtractive, 2026-07-14): BookDetail renders every section fully expanded, always — all collapse/disclosure deleted (CollapsibleSection usages, Files chevron toggle); the Files Edit ⇄ Done toggle survives (it gates destructive actions, not disclosure). Section ORDER is deliberately untouched — that's the next session's decision, made against the now-honest screen. v0.59.0 (S16 Status Knot, Session B1 — backend, 2026-07-15): reading status is sessions-canonical — titles.status/date_started/date_finished/rating are a projection of reading_sessions recomputed in-transaction on every session write; the status/rating/dates endpoints write sessions, never title columns; the date_finished clobber and the backfilled-read status steal are dead at the projection layer. v0.60.0 (S16 Status Knot, Session B2 — frontend, 2026-07-15): BookDetail's action surface is an inline 4-state segmented toggle (status-token colors, custom labels wrap 2×2, 44px targets) with inline Finished date+rating capture and a download-triggered "Start reading?" prompt for Unread titles; the status pill is display-only; ReadingStatusCard + both status modals evicted from BookDetail (modals survive for list views, all on the one-call contract rendering the returned projection; snap-back microcopy for the two ratified Unread cases); dead date-editor wiring and consumer-less api.js `updateBookDates`/`updateBookRating` deleted grep-verified; two of the 13 ledgered `alert()` sites fell with the rewritten CollectionDetail handlers (11 remain for batch 3). S16 Status Knot is closed. v0.61.0 (Session B3 — menu audit + History fixes, 2026-07-15): the BookDetail 3-dot menu is page-level actions only (Edit · Change Cover ─ Merge · Rescan Metadata ─ Delete Title — add-session and add-to-collection removed; both actions already live in their sections); Reading History's header button is a + glyph (it always added a session — the pencil misrepresented it); the session list renders in projection order (`date_started DESC, id DESC`, matching `sync_title_from_sessions` — the top row IS the status pill's source); Times Read counts finished sessions only (in-progress/DNF never count); the remove-format confirm is honest about files (header "Remove Format"; the trash sentence is suppressed for stale paths and within-title shared files — cross-title sharing parked as an accepted edge). v0.62.0 (2026-07-16): upload replace is contained — the client-named existing folder now routes through the `_trash/` move contract (containment + symlink refusal live in the trash service; any rejection aborts the replace before new files are written and surfaces as the per-book error) instead of an unguarded `rmtree`, closing the last destructive file operation outside the trash contract; the dead import router (still querying the pre-Phase-5 `books` table) is unmounted and deleted together with the orphaned, nav-less ImportPage that called it — the batch-3 parked item pulled forward and closed full-sweep (ImportPage's 4 ledgered `alert()` sites fell with the page; 7 remain). v0.63.0 (2026-07-17): the `alert()` ledger is closed — the final 7 sites (CollectionsTab 1, CollectionModal 4, CollectionDetail 2) converted to inline `role="alert"` banners in their host surfaces (delete-confirm modals, the cover-style controls, the remove-mode bottom bar), each with error state + clear-on-retry; the design-lint alert category records its first 0/pass (the rule has been strict since the v0.57.0 rider — no rule change was needed) and every strict category now passes; 6 new error strings joined the batch-close microcopy queue. v0.64.0 (2026-07-17): the batch-remove retry trap is closed at the root — `DELETE /collections/{id}/books/{title_id}` is idempotent (removing a non-member is a success no-op; unknown-collection 404, automatic-collection 400, and checklist completion's membership 404 all preserved), so a partial batch-remove retry drains clean and CollectionPicker unchecks survive state drift; 9/9 end-to-end harness checks against the real schema. v0.65.0 (2026-07-17): three ratified UI fixes — StatusLabelsModal field titles render static canonical defaults with a per-field Reset link riding the modal's save-on-blur path (the live-label row titles are gone; "Reset" string queued for the microcopy pass); the BookDetail 3-dot bottom sheet portals to body so it stacks above the bottom nav (root cause: the sticky z-20 UnifiedNavBar stacking context trapped the z-50 sheet under the z-40 nav; first `createPortal` in the codebase, with a second outside-click containment ref so portaled sheet taps don't close the menu early); the Files section Edit ⇄ Done gate is deleted — reversing the v0.58.0 "toggle survives" call per the 2026-07-16 ratification — with the per-row remove × always visible at 2+ editions, muted at rest, confirm modal and v0.61.0 in-flight guards unchanged. Roadmap record corrected in the same pass: 10.0.14's ThreeDotMenu extraction was checked but never done — the component is still inline in BookDetail. v0.66.0 (Batch 3 B1, 2026-07-17): `_trash/` gets an in-app home — a Settings Trash section (top-level item count + recursive size; "Trash is empty." zero-state renders no button) with `[Empty trash]` behind a type-forever-to-confirm modal that owns the app's ONLY truly irreversible operation: the copy states plainly that emptied files skip the server's recycle bin and that backups cover the library database only (the Backups section now carries the same database-only clarifier). New `GET /api/trash/stats` + `POST /api/trash/empty` reuse the trash service's single `_trash` definition, refuse symlinked or non-contained trash resolutions before touching anything, delete per top-level entry without aborting on failures (failures return in-band with 200, per the v0.62.0 contract; a symlink in trash deletes the link, never its target), log every removed entry, and never touch the database; failure keeps the modal open with an inline banner and refetches stats so the shown count stays honest. 29/29 end-to-end harness checks. v0.67.0 (Batch 3 B2 — small-fix sweep, 2026-07-17): unknown URLs land on Library via a catch-all route; desktop downloads fetch first, so a stale-path edition surfaces the backend's honest 404 detail as an error toast instead of failing silently (the no-share and share-fallback branches unified into one blob-save path — the stale row itself stays by-design, per the drift-check scope ruling); upload results render the backend's per-book skip/defer messages (S15.2b carry-over closed); `add_format` uploads are contained — the client-supplied folder resolves symlink-safe inside the books root (never the root, never `_trash/`) and destination collisions are refused, per-book and pre-flighted, never overwritten and never a mid-batch 4xx; backup rotation now parses `liminal_pre_sync_*` names (timestamp from the trailing tokens, `liminal_` prefix required), so pre-sync backups age out under daily retention instead of accumulating forever — the un-specced "backup-filename parser" item closed by characterizing the one real defect per the 2026-07-17 ruling; recon confirmed the upload sync-completion log was already fixed and `getBackLabel`'s `/sync-results` case still shipped (v0.57.0 rider) — both no-change. v0.68.0 (Batch 3 B3 — upload write-path containment, 2026-07-17, chip-spawned from the B2 A1 finding): every upload write path is contained at two ratified layers — intake keeps only the leaf of the client filename (browsers send bare basenames; folder upload isn't offered, so legitimate traffic is untouched) AND each path pre-flights resolved destinations before its first write (also refusing symlink-at-leaf redirects); `'new'` uploads refuse filename collisions per-book instead of silently overwriting an existing folder's files (deliberate swaps go through the review screen's Replace, which trashes first), leave no stray folder on rejection, and refuse pre-planted symlink redirects of the folder name; the one-tap wishlist Add Files flow now uses its sibling's recorded rename-on-collision contract (`name_1.epub`, nothing replaced — a linked title is a normal title, ratified 2026-07-17) — 22/22 end-to-end harness checks; a user-reported notes/covers loss on some other wishlist-conversion route is parked as its own investigation. v0.69.0 (Batch 3 B4 — wishlist-conversion data loss, 2026-07-18): the parked investigation ran and both ratified fixes shipped — familiar-title matching now includes wishlist entries (they have no folder, so the folder-duplicate check could never catch them, and the old query excluded them outright — every upload of a wishlisted book created a duplicate title whose cleanup destroyed notes and covers), the review card labels the match ("is on your wishlist… will move it to your library"), adding files to a wishlist title converts it to owned exactly like the Acquire flow, and merge now carries the source's cover under the ratified rule (real beats none, custom beats extracted, a custom target is never overridden) so merging a wishlist/owned duplicate pair is finally lossless for notes AND cover; 17/17 end-to-end harness checks; the loss routes, the safe routes, and the already-lost-data honesty (backup-only recovery) are recorded in the changelog. v0.70.0 (D-Fix session, 2026-07-19): the three Diagnostic-v2 ratifications shipped — merge converts the source's wishlist note (`tbr_reason`, a titles-row column that previously died with the row — the cover bug one column over) into a real note on the kept title with the "Why this one (from the wishlist): " prefix; wishlist familiar matches start unresolved on the review screen (the chooser, attention banner, and Add gate were already built and simply never armed — the initializer auto-resolved every familiar match to 'new'); both upload routes refuse same-format duplicate files per-file with messages that point at the fix (nothing lands as an unreferenced `_1` orphan anymore, the linkTo flow reports through its real response instead of a fabricated one, and an all-refused link leaves a wishlist title unconverted); and the Files section gains the per-format [Replace file] action behind an inline confirm — trash-first swap, server-derived paths, extension must match the slot, collisions refused before anything is trashed, edition row re-pointed and re-stat'd, metadata never touched. 17/17 end-to-end harness checks; design-lint all-strict-pass with a zero raw-button delta. The queued renames + list-view silent-failure session (B5) moves to v0.71.0 — data-loss and honesty fixes outranked it. v0.71.0 (aftermath fixes, 2026-07-19): production testing caught Replace file firing its failure string on swaps that landed — diagnosed as a real masked failure, not a cosmetic one: the edition row update wrote a column that doesn't exist (edition sizes are stat'd at read time, not stored), so the write raised after the trash and land steps had succeeded, and same-name swaps hid the never-updated row; the update now writes the path only, proven end-to-end against the real schema (the earlier harness had hand-rolled the column into existence — harnesses now build their DB from the real schema initializer). The Done page header now reflects the outcome — zero adds renders "Nothing was added" with the attention glyph instead of a green check over an error row, partial success states honest counts; the wishlist-match chooser action reads "Move to library" (ratified frozen copy edit, one line, zero hex); the merge note prefix is v2 past-tense. B5 renames now target v0.72.0. v0.72.0 (Batch 3 B5 — honest names + list-view silent-failure audit, 2026-07-19): both ratified renames shipped — ReadingStatusCard → AcquireCard (wishlist-only since v0.60.0; its dormant download/session-action props are reported as a dead-code candidate, not deleted) and ui/CollapsibleSection → TruncatedText (it truncates behind a gradient fade with View more; it never collapsed a section — the C-pass lint item now targets TruncatedText.jsx); the silent-failure audit converted every blocking console-only catch in the list-view surfaces to inline `role="alert"` surfaces at the point of action — CollectionPicker's failed load no longer renders the lying "No collections yet" empty state and its failed toggle/create-then-add now speak, StatusLabelsModal's failed load offers retry and a failed save-on-blur or Reset rolls back the optimistic value and reports at the field row (the UI never shows a default the database doesn't hold), CollectionDetail's three infinite-scroll sentinels end with a retry row instead of silently truncating, and both drag-reorder surfaces explain the snap-back — benign sites (localStorage guards, reading-speed/category prefetches) stay silent with recorded justifications, and the one surface needing cross-file threading (FilterDrawer's category options) is parked per the audit's stop rule; the SortDropdown duplicate bottom sheet is verified sound on all four rendering pages — no ancestor creates a stacking context, so the in-tree fixed sheet joins the root context above the bottom nav and the v0.65.0 ThreeDotMenu trap (a sticky stacking-context ancestor) structurally cannot apply — recorded so the duplicate-implementation note stops resurfacing as a stacking suspicion; the dedup itself (SortDropdown sheet vs ThreeDotMenu's portaled sheet) stays a candidate for a session that names it. v0.73.0 (Batch 3 C1 — code, 2026-07-20): the v0.72.0 dead-code candidates are gone — ui/TruncatedText deleted outright (gallery-demo-is-not-a-consumer ratified 2026-07-20; its stale §3 doc entry, still titled CollapsibleSection, deleted with it) and AcquireCard's dormant download/session-action block deleted grep-verified, leaving the wishlist Acquire card behavior-identical; IconButton grows a proper variant="muted" and the two Files-row `!` important overrides die; StatusLabelsModal's failed field takes the danger border per the Forms pattern; design-lint gets two integrity fixes — comments are stripped before matching (the raw-button baseline was 1 high since S13 from a JSDoc `<button`; true baseline 124, now 120 after the AcquireCard deletions) and the window.confirm rule now catches bare confirm( (zero call sites today; pure hardening). Two stop conditions honored as reports: the Settings → component-gallery link did NOT ship (the /dev/components route has no working mobile exit — no bottom nav outside the app shell, no page-level back affordance; the link ships together with an exit in a future session), and the unbracketed-hex lint tightening did NOT ship (the tightened rule surfaces 32 sites — over the 15-site stop threshold — most of them gradient/badge generation *data* rather than styling; the rule's scope needs ratification before ignores are written). C2 (docs errata + microcopy pass) is next.
