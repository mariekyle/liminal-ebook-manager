...
**Discovered in Batch 1:** `DuplicateFinderModal.jsx` was imported nowhere (dead since the DuplicatesPage rework) — verified repo-wide and **deleted** same session. Voice-pass candidates left as-is (out of scope): DuplicatesPage:183 "Your library is clean!" exclamation, :196 "books" in the duplicate-count string. (Both resolved by the v0.85.0 voice riders.)
...
| S4b | Matcher rescope (B3a element-children OR role="option"), role additions, SettingsRow move to ui/, BookLinkPopup close ×→IconButton, chrome markers (incl. SortDropdown's 9 provisional), **raw-`<button>` STRICT FLIP**, sprint scorecard | ✅ Shipped v0.82.0 — **SPRINT COMPLETE.** Raw-button strict at ZERO: 47 annotated chrome markers (9 provisional-SortDropdown, 2 ratified-bespoke segment groups, 3 honesty annotations, 33 plain chrome), 20 structural exemptions under the two written criteria (+2 matcher tightenings caught over-exemption: nested-button both directions, /Icon$/ components), zero unmarked, zero ignore lines, all ten lint categories pass. Riders landed: 44px trigger, token Cancel, eighth Badge site, SettingsRow→ui/ (sixteen ui/ components), BookLinkPopup ✕→IconButton. v0.81.0 phone defect fixed: popup w-max + the save-✓ (not residue — kept, placed beside the chip) |

### Merge-Confirm Redesign (decisions locked 2026-07-25)

**Status:** ✅ CLOSED — both sessions shipped (v0.84.0 BookDetail, v0.85.0 DuplicatesPage). Duplicate-pair triage moves to the front of the queue.
**Sessions:** 2 — the BookDetail modal, then DuplicatesPage (the session-2 prompt is written after session 1 lands, per the decision)

| Session | Scope | Status |
|---------|-------|--------|
| 1 | BookDetail confirm redesign: Keeping-first cards (SOURCE/TARGET boxes + arrow + static lists deleted), computed "What happens" rows with zero-suppression, conditional trash line (`editionFileOnDisk` per edition), metadata-honesty caption, select-time target fetch (never blocks the merge), post-merge toast from the previously-discarded response (counts + conditional carries), conditional search-step caption + riders (mergeTitles JSDoc, token classes in touched blocks, SortDropdown gallery demo) | ✅ Shipped v0.84.0 — new strings flagged for ratification in the session report |
| 2 | DuplicatesPage: the ratified button pair reaches every occurrence ("Merge & Delete" / "Cancel" / "This can't be undone." retired), find_duplicates serializer extension (per-book `has_file` stat-checked + `session_count` + `note_count` — STOP-gated on query cost), computed confirm content in the inline row, numbered success state from the response, bulk partial-failure auto-rescan (merged M of N, then failed — no stale rows) | ✅ Shipped v0.85.0 — serializer additive (STOP gate did not fire, harness 7/7), gallery 17 of 17 via the SettingsRow rider, new strings flagged for ratification in the session report |

**Deferred, recorded 2026-07-25:** the duplicates scan can't see wishlist entries — a future enhancement optionally includes them in the scan, always as the merge-away side (wishlist-as-target stays impossible by design: the owned-only target search is intentional).
