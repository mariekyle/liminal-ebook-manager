...
function ShipModal({ isOpen, onClose, selectedShip, onApply }) {
  ...
  // Filter ships by search
  const filteredShips = ships.filter(ship =>
    ship.toLowerCase().includes(search.toLowerCase())
  )
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-library-card rounded-lg w-full max-w-md max-h-[80vh] flex flex-col">
        ...
        <div className="flex items-center justify-between p-4 border-b border-gray-700">
          <h2 className="text-lg font-semibold text-white">Filter by Ship</h2>
          <button 
            onClick={onClose}
