...
- **Startup snapshot (D-009).** `main.py`'s lifespan copies `library.db` through the backup service (`backup_type="pre_deploy"`, filename `liminal_pre_deploy_YYYYMMDD_HHMMSS.db` in the daily folder, so retention ages it out) before `init_db()` runs. No DB file → warning, skipped. Backups disabled in Settings → warning stating explicitly that no snapshot was taken, startup continues. Snapshot failed → startup aborts (D-009 rider: `create_backup` returns a status rather than raising, so the status is checked).
- **Library sentinel (D-008).** `LIBRARY_SENTINEL = ".liminal-library"` in `backend/constants.py`. `/api/health` gains `library_mounted` and reports `status: library_unreachable` when the marker is missing at the books root. `_do_sync` refuses with the message its missing-path branch already uses ("Couldn't reach the library folder. Check that it's connected, then try again." — not yet in `docs/MICROCOPY_LIBRARY.md`; pre-existing gap, flagged) when the marker is missing; both `/api/sync` and the upload-triggered background sync pass through `_do_sync`. **Before the next rebuild, create the empty file `.liminal-library` at the root of the book library, or sync will refuse.**
- **PWA shell (D-013).** `vite-plugin-pwa` (`registerType: autoUpdate`, `injectRegister: auto`): web manifest (`Liminal`, standalone, `start_url: /`, `theme_color` `#242220` and `background_color` `#1a1918` from the tokens), icons 192 / 512 / 512-maskable generated from `frontend/public/icon.png`, a service worker that precaches only the built assets. `/api/*` is on the navigation-fallback denylist and there is no runtime caching, so no API response is ever served from the worker. `index.html`'s `theme-color` moves from the dead navy `#0f172a` to the surface token.

### Changed
- **`docker-compose.yml` is a run file (D-012).** Single `liminal` service on `${LIMINAL_IMAGE:-ghcr.io/mariekyle/liminal-ebook-manager:stable}`, `pull_policy: always`; `DATA_HOST_PATH`, `BOOKS_HOST_PATH` and `BACKUPS_HOST_PATH` are required (`:?`) so it fails loudly with any unset and carries no path. `BOOKS_DIR=/books` added beside `BOOKS_PATH` (upload.py reads the second name). No `build:`. `.env.example` documents the three paths and the optional `LIMINAL_IMAGE` for a local build.
- **Static serving in `main.py`.** The SPA catch-all now serves a file that exists directly inside `static/` (`sw.js`, `manifest.webmanifest`, `registerSW.js`, `workbox-*.js`, the icons) as itself instead of returning `index.html` for it; without this the service worker would register against an HTML body and fail silently. Only bare filenames qualify, so nothing can path-traverse.
- `scripts/lcheck.sh` mirrors `frontend/public/` too (icons ship with the image build; ruling 2026-09-24 — widening the scope is not retiring the script, D-011).
- Version `0.87.0` → `0.88.0`.

### Technical
- Created: `.github/workflows/publish.yml`, `.github/workflows/promote.yml`, `.dockerignore`, `frontend/public/icon.png` (Marie), `frontend/public/icon-192.png`, `frontend/public/icon-512.png`, `frontend/public/icon-512-maskable.png`.
- Modified: `backend/main.py`, `backend/routers/sync.py`, `backend/constants.py`, `scripts/lcheck.sh`, `docker-compose.yml`, `.env.example`, `frontend/vite.config.js`, `frontend/index.html`, `frontend/package.json`, `frontend/package-lock.json` (`vite-plugin-pwa` ^1.3.0 dev dependency), `CLAUDE.md`, `docs/DECISIONS.md`, `docs/PIPELINE.md`, `ROADMAP.md`.
- Frozen files untouched. No schema change; `database.py` read-only. **Backend changed: full rebuild required.**
- Verified: `npm run build` clean (PWA precache 11 entries, no `/api` route); `python -m compileall backend` clean; `create_backup` and `LIBRARY_SENTINEL` import↔export grepped both sides; the real `create_backup` exercised against a scratch copy of the production DB for the enabled, disabled and failure cases; `promote.yml` byte-identical to todo's.

### Changed (workflow, 2026-09-23/24, shipped in `20619be`)
- **Dev workflow brought in line with the todo repo.** `docs/DECISIONS.md` gains numbered `## D-NNN · date · decision` blocks and a Pending-ratification section (D-002; earlier entries untouched). `docs/OPEN_QUESTIONS.md` gains an INBOX section for raw phone drops, triaged at session start. `.githooks/pre-commit` chains the existing design-lint hook and then rejects any app-code commit that changes nothing in `docs/`, `CHANGELOG.md` or `ROADMAP.md` (D-001). `CLAUDE.md` is now tracked (previously gitignored); machine-specific detail moves to the gitignored `CLAUDE.local.md`. `.gitignore` covers `.claude/settings.local.json`, `.claude/launch.json`, `.claude/worktrees/` and `CLAUDE.local.md`.
...
### Changed
- **Group header restructured into two fixed rows (`frontend/src/pages/DuplicatesPage.jsx`).** Row 1 is metadata only — match-type badge, conditional Same Author badge, "{n} titles" — nothing tappable; badges may wrap among themselves at narrow widths, actions never do. Row 2 is the verdict row: `flex items-stretch justify-between gap-2 min-h-[44px]`, "Not duplicates" left-anchored and "Merge into selected" right-anchored in every group regardless of badge count — replacing the single `flex-wrap justify-between` header whose action position depended on how the badges wrapped (the nondeterministic-placement finding). Both actions stretch to the row's 44px, so the touch height comes from the row while the buttons keep their shipped `sm` styling; labels, variants, handlers, and disabled/loading wiring are byte-identical. The success chip re-homes into the same row-2 slot (`mt-3 flex items-center min-h-[44px]`).
- **Unwrapped-at-~380px, stated honestly as arithmetic:** worst-case combined width ≈305px (0.56em/char bound at 13px: "Merge into selected" ≈167px + "Not duplicates" ≈130px + 8px gap) against 314px of row width (380 − 32 page padding − 2 card borders − 32 header padding), and the row is structurally wrap-proof — no `flex-wrap`, `whitespace-nowrap` labels. A real-render measurement was attempted and abandoned: the Browser pane renders out-of-project files as non-scriptable snapshots and localhost origins are policy-blocked without a repo `launch.json`, which this session declined to create.

### Fixed
- **The ratified carry-over pair: both banners tokenize (v0.85.0 review, MINOR).** Root cause: the scan-error banner and the bulkFailure banner carried the danger color tokens but **no typography token** — their text rendered on inherited default sizing, the token-pairing rule broken from the type side. Each gains exactly one class, `text-body-sm`, landing both on the v0.63.0 canonical banner set (`rounded-lg … text-body-sm bg-action-danger/10 border border-action-danger/30 text-action-danger`); they still match each other, copy untouched. `role="alert"` deliberately NOT added — that is the separately-tracked live-region debt family (settings-banner gap), a behavior change beyond this session's styling scope; flagged, not smuggled.

### Technical
- **STOP record (§2 variant promotion, ratified STOP honored):** "Not duplicates" was to change ghost → "the bordered secondary/default Button variant". Button's variant API (`ui/Button.jsx`): `primary` (#5e8a8a fill), `secondary` (#4a4541 fill), `ghost` (transparent), `danger`/`success`/`warning` (fills) — **no variant carries a border utility, so no bordered non-primary, non-danger variant exists.** Nothing invented, no inline styles; the control ships ghost and the missing-signifier finding stays open pending a Button-variant decision (a bordered variant is a shared-component change felt at every Button surface — its own ratification).
- **Modified:** `frontend/src/pages/DuplicatesPage.jsx` (header restructure + two one-class banner additions — the only three edit sites), `backend/main.py` (0.86.0 → 0.87.0), `CHANGELOG.md`, `ROADMAP.md`. Dismissed-pairs panel, confirm row, book rows, help strip: zero diff — the restructure forced nothing, reported per constraint.
- **Verification:** design-lint FULL strict all ten categories 0/pass; esbuild parse exit-0 (deno npm shim); copy audit by construction — every user-visible string in the three edit hunks carried byte-identical, audited per-edit because the tree-vs-HEAD diff still contains all of uncommitted v0.86.0; frozen six untouched this session (`database.py`'s tree diff is v0.86.0's pre-ratified migration, unchanged today).
- **Backend files changed (main.py version line only): full Docker rebuild still required. No schema change, no migration, no bulk writes — no `library.db` backup this time.**

## [0.86.0] — 2026-07-26

**Dismiss-pair mechanism — the answer to 76 false-positive groups (~95% of scan results). Implements the six decisions ratified 2026-07-26: every duplicate group gains a "Not duplicates" control that records all its constituent pairs as dismissed; dismissed pairs are filtered inside the matcher at two points (D3 — inline guard at the fuzzy pairwise test, connected-components split per exact bucket; the uniform post-pass was rejected because fuzzy groups are greedy stars whose non-seed edges were never actually compared), so counts and the clean state are post-dismissal by construction (D6). A "Dismissed pairs ({n})" panel below the results lists stored pairs and restores per pair — dismiss in groups, restore in pairs, asymmetry honest to storage (D5). Pairs are unordered title-id pairs (D1): stale pairs referencing merged or deleted titles go inert and drop out of reads via JOIN, never cascaded.**

### Added
- **`dismissed_duplicate_pairs` table (`backend/database.py` — FROZEN FILE, edit PRE-RATIFIED in Decisions 2026-07-26/D2).** Created in `run_titles_migrations` after the S15 edition-relabel block, following the Phase 9A `backup_history` shape exactly: idempotent `CREATE TABLE IF NOT EXISTS` (`title_id_a`, `title_id_b`, `dismissed_at`, PK on the pair) + `CREATE INDEX IF NOT EXISTS` on `dismissed_at`. `a < b` is enforced at write time in the router, not with a CHECK constraint, and there is no FK cascade — both by decision.
- **Three endpoints (`backend/routers/titles.py`).** `POST /titles/duplicates/dismissed` accepts one group's `title_ids` (embed-Body, the `merge_titles` idiom), expands to all C(n,2) pairs, normalizes `a < b`, `INSERT OR IGNORE` in one transaction — re-dismissal is idempotent; fewer than 2 distinct ids → 400. `GET /titles/duplicates/dismissed` resolves pairs via double INNER JOIN to `titles` (id + title + authors per side) — stale pairs excluded by construction, newest first. `DELETE /titles/duplicates/dismissed/{a}/{b}` restores one pair, ids accepted in either order, idempotent (`restored: false` instead of an error — the v0.64.0 retry-trap lesson).
- **Matcher filtering (D3, two-point).** `find_duplicates` loads the dismissed set once per scan. Fuzzy pass: a guard at the pairwise similarity test skips dismissed pairs WITHOUT claiming the candidate — it stays eligible to seed or join a later group (harness-proven: A–B dismissed, B–C similar → {B,C} forms). Exact pass: each bucket is a complete graph, dismissed edges drop, and connected components of 2+ survive as groups (`match_type` stays `exact`, `same_author` recomputed per component); members left in no component fall through to the fuzzy pass unclaimed. With nothing dismissed the whole bucket is one component — output identical to v0.85.0 (harness S0 baselines).
- **DuplicatesPage: "Not duplicates" per group** — ghost variant (quiet, not danger: it destroys nothing), left of "Merge into selected", hidden during confirm/success, built-in spinner while in flight. No inline confirm by decision — dismissal is non-destructive and reversible via Restore. On success the group leaves the list immediately with no rescan; the summary count follows (the merge-removal arithmetic, minus the timer). Failure lands in a per-group error row (same slot styling as merge errors; the two never coexist).
- **DuplicatesPage: "Dismissed pairs ({n})" panel** below the results — hidden entirely at zero, renders in both the clean state and the results state. Expands (chevron, `aria-expanded`) to the stored pair list: both titles + authors per row, "Restore" per row with per-row failure captions. Restore removes the row locally; the pair is eligible next scan — deliberately no auto-rescan. The list refetches alongside every scan and after every dismissal, so the count is backend truth (merges delete titles; stale pairs drop out on the next read). Load failure renders an inline error + "Try again" instead of hiding the surface.
- **api.js: three wrappers** — `dismissDuplicateGroup`, `getDismissedDuplicates`, `restoreDismissedPair`. **SCOPE ADDITION, flagged as a made call:** api.js was not in the prompt's file list, but `apiFetch` is module-private, and duplicating its base-URL + error-sanitization logic inside the page would have been worse. Three additive functions in the exact `smartPasteApply`/`mergeTitles` idiom.

### Changed
- `find_duplicates` exact pass restructured from bucket-append to component emission (behavior identical with an empty dismissed table — harness S0); serializer shape, per-book field order, sort (exact-first, larger-first), and `total_duplicates` arithmetic all unchanged (harness S7/S8).

### Technical
- **Modified:** `backend/database.py` (migration block only), `backend/routers/titles.py` (`dismissed_key` helper, dismissed-set load, exact-pass components, fuzzy guard, three endpoints), `backend/main.py` (0.85.0 → 0.86.0), `frontend/src/api.js` (three wrappers), `frontend/src/pages/DuplicatesPage.jsx` (three handlers, seven state slots, header control, dismiss error row, dismissed panel; the two untokened banners — scan-error, bulkFailure — untouched per their ratified carry-over).
- **Verification:** venv harness against the REAL `init_db` schema (never hand-rolled, per the v0.70.0 lesson) — **23/23 PASS** covering pair normalization, C(3,2) expansion + OR IGNORE idempotence, 400 validation, fuzzy re-seeding, exact-component survival, full dismissal (fuzzy guard holds the pair out too), restore round-trip + idempotent second restore, stale-pair GET exclusion + inert-row retention, serializer field order, exact-first sort, and the has_file stat path. design-lint FULL strict **all ten categories 0/pass**, report unchanged. `py_compile` clean (titles.py, database.py); esbuild parse exit-0 (DuplicatesPage.jsx, api.js); import↔export grep both sides — 5/5 imports matched to exports, matches printed. Frozen check via git: only database.py among the six, pre-ratified. End-of-session review (code-reviewer, three checks): frozen PASS, scope PASS, pattern conformance 1 finding — the dismissed-list LOAD failure wore the danger register where DESIGN_SYSTEM §4's two-register rule prescribes quiet (`text-text-secondary`) for load failures; fixed in-session (one token), action-failure surfaces keep danger, esbuild + lint re-verified 0/10.
- **Ratified strings used verbatim:** "Not duplicates" · "Dismissed pairs ({n})" · "Restore".
- **New strings flagged for ratification:** "Couldn't load dismissed pairs." · "Couldn't dismiss this group. Try again?" · "Couldn't restore this pair. Try again?" (all three follow the MICROCOPY_LIBRARY load/action-failure formulas) · backend 400 detail "A dismissal needs at least two distinct titles." (unreachable through the shipped UI — groups always carry ≥2 ids). Reused, not re-flagged: "Try again" (already this page's scan-error retry label).
- **Made calls flagged:** the api.js scope addition (above) · dismissed list refetched per scan/dismissal rather than maintained by client arithmetic (truth over thrift) · exact-pass singleton components fall to the fuzzy pass unclaimed (symmetric with D3's re-seed rationale; no-dismissal output proven identical) · in-flight dismiss shows Button's built-in spinner, no new progress string · `dismissed_at` returned by GET but not rendered in v1 (it is the list's newest-first order key).
- **Backend files changed: full Docker rebuild required. SCHEMA CHANGE — back up `library.db` BEFORE copying files and rebuilding** (Container Manager: stop container → copy the db file → start). The migration is idempotent (`IF NOT EXISTS`), creates one empty table + one index, rewrites no data, and has no destructive step.

## [0.85.0] — 2026-07-25

**Merge-confirm redesign, session 2 of 2 — DuplicatesPage. Implements the DuplicatesPage half of the 2026-07-25 Decisions: the inline confirm row stops asserting a static operation and describes the computed outcome aggregated across the merging-away titles, the ratified button pair ("Merge" / "Keep separate") reaches its last surface — "Merge & Delete", "Cancel", and "This can't be undone." are retired from the merge flow repo-wide — the group rows gain real triage data (per-title read/note counts + a stat-checked no-file indicator), the success state reports what actually moved from the previously-discarded merge responses, and a mid-loop failure auto-rescans for truth and names what completed. Voice riders land page-wide (books→titles, click→tap, emoji strip, exclamation cleanup). Backend: `find_duplicates` gains three per-book fields, additions only.**

### Added
- **`find_duplicates` serializer extension (`backend/routers/titles.py`, additive only — the STOP gate on query restructuring did not fire).** Each book in each group now carries:
  - `session_count` / `note_count` — two subqueries added to the existing titles query, shaped exactly like the `edition_count` subquery already there (`reading_sessions` and `notes` both carry `title_id` indexes; the same COUNT queries already run per-merge in `merge_titles`).
  - `has_file` — stat-checked per the editions-serializer idiom (`os.path.getsize` in try/except, :343–348): a title counts as having a file only when at least one edition's `file_path` stats successfully. Zero editions → `false`; only dead paths → `false`. Computed in a post-pass over the finished groups, so **only titles that actually landed in a duplicate group are stat-checked** — never the whole library.
  - Backward compatible: the original 7 per-book fields are byte-identical in name and type; group and top-level shapes unchanged. Runtime-verified by a venv harness against an `init_db` schema — 7/7 assertions (live-file/dead-path/zero-edition branches, wishlist exclusion, field order, types).
- **Group rows show the triage data.** Each title's caption line gains `{n} read(s)` and `{n} note(s)` after the edition count (ratified parts vocabulary; zeros render deliberately — "0 reads · 0 notes" vs "12 reads · 3 notes" is the triage comparison), plus `no file on disk` (the v0.84.0-ratified string) when `has_file` is exactly `false`. Rows with a file show no file caption — presence is the default, absence is the signal. Edition-count display unchanged.
- **Numbered success state.** The merge loop stops discarding its response bodies: `merged.sessions/notes/collections` sum across the N−1 merges, and the success chip reads `Merged — {parts} moved over` in exactly the v0.84.0 toast vocabulary (zero counts suppressed; plain `Merged` when nothing moved; `cover carried` / `wishlist note carried` appended when any response reports them). The 1.5s removal behavior is unchanged (not relitigated this session).
- **Bulk partial-failure (Decisions 2026-07-25).** A mid-loop merge failure stops the loop, auto-triggers the existing duplicates rescan (truth restoration — completed merges deleted titles, and the page must never render rows that no longer exist), and surfaces `Merged {M} of {N}, then failed`. The message lands in the surviving group's error row when the post-rescan results still contain a group with the kept title; if the group is gone (its remaining titles stopped matching), it lands in a new page-level banner in the top error position, cleared by the next scan. Completed merges stay — no rollback pretense. `scanForDuplicates` now returns the keyed groups so the failure path can locate the survivor (additive; other callers ignore the return). The underlying error detail still goes to the console; the ratified numbered string is the user-facing surface (made call, flagged below).
- **ComponentPreview section 17: SettingsRow** — the gallery gap flagged in v0.84.0's arithmetic correction closes; **all 17 shared `ui/` components are now demoed.** Demos cover the three types (navigation/toggle/display) plus loading and disabled, with a live toggle and a contract caption. Import↔export grep both sides: default import (ComponentPreview.jsx:19) ↔ `export default function SettingsRow` (ui/SettingsRow.jsx:28).

### Changed
- **Confirm row describes the computed outcome (ratified mockup).** The question keeps its shape — `Merge {N} title(s) into "{kept}"?` — and the static "This can't be undone." is replaced by aggregated sums across the merging-away titles: `{X} read(s) and {Y} note(s) move over.` (zero rows suppressed, verb by total — the v0.84.0 idiom) plus a trash clause **only when at least one merging-away title has a file on disk**: `{F} file(s) go(es) to the trash folder.` When everything is zero and fileless the sentence is just the question. Missing count fields degrade to zero (`|| 0` guards) — the row falls back to the bare question rather than rendering `undefined`.
- **Buttons: "Merge" (danger, sm) / "Keep separate" (ghost, sm)** — the ratified pair, replacing "Merge & Delete" / "Cancel". Same positions (safe option first), same sizes.
- **Voice riders, page-wide:** summary count `({N} books)` → `({N} titles)`; group header `{n} books` → `{n} titles`; validation error `Please select a book to keep` → `Select a title to keep` (pre-commit ratification round trimmed the "Please "); view-link tooltip `View book details` → `View title details`; help strip 💡 stripped and rewritten `Select the title to keep, then tap "Merge into selected". Other titles will be merged into it.` (click→tap, books→titles); empty state ✨ emoji div deleted and `Your library is clean!` → `Your library is clean.` (exclamation cleanup — replacement flagged verbatim below, not invented silently); success `Merged!` superseded by the numbered chip (exclamation dies with it). **Trigger label ratified sentence-case in the pre-commit round: `Merge into Selected` → `Merge into selected`** (button + its help-strip quote — the session flagged as-is vs sentence-case; Marie ratified the alternative before commit).

### Fixed
- **Stale success-timer could corrupt a fresh scan's count.** Root cause: the 1.5s removal timer captures its group and key; if a rescan replaces the groups before it fires (manual Rescan inside the window — or now any partial-failure auto-rescan), the timer's functional update filtered nothing (fresh keys don't match) **but still decremented the fresh scan's `total_duplicates`** by the dead group's size, skewing the summary line. The auto-rescan path widened this pre-existing race from unlikely to plausible, so the timer now no-ops entirely when its key is absent from current results. Visible behavior on the normal path is identical.
- **Merging-flag cleanup no longer resurrects dead keys.** The old `finally` re-added `merging[key] = false` unconditionally; on the new failure path the rescan has already wiped every per-group map and issued fresh keys, so the finally would have written a stale-key entry into the fresh state. The clear now happens explicitly in the success path only (same visible behavior); the failure path's state reset is the rescan itself.

### Removed
- **From DuplicatesPage:** `Merge & Delete`, `Cancel` (confirm row), `This can't be undone.`, the 💡 and ✨ emoji, and both exclamation strings. Grep proofs, live tree: `Merge & Delete` **0 repo-wide**; 💡/✨ **0 in DuplicatesPage**; `This can't be undone` **0 in DuplicatesPage and 0 on any merge surface** — the two remaining repo hits are BookDetail **delete** flows (session-delete confirm :2907; Delete-Title zero-history fallback :3418), truthful irreversibility claims on genuine DB deletions, outside decision 30's merge-string scope and inside this session's zero-diff file. Flagged in the session report rather than silently swept.

### Technical
- **Modified:** `backend/routers/titles.py` (find_duplicates only: 2 subqueries, 2 serializer-site additions, has_file post-pass), `backend/main.py` (0.84.0 → 0.85.0), `frontend/src/pages/DuplicatesPage.jsx` (confirm row, success accumulation, partial-failure path + `bulkFailure` state, row captions, voice riders), `frontend/src/pages/ComponentPreview.jsx` (SettingsRow import + toggle state + section 17), `CHANGELOG.md`, `ROADMAP.md`. **BookDetail.jsx zero diff** (the Toast live-region rider's fix proved structural — reported, not implemented, per its STOP condition). `docs/DESIGN_SYSTEM.md` verified as NOT recording gallery coverage (zero gallery/ComponentPreview mentions) — no edit.
- **Verification:** design-lint FULL strict all ten categories 0/pass, chrome inventory unchanged at 38, zero ignore lines; esbuild parse exit-0 on DuplicatesPage.jsx + ComponentPreview.jsx; `py_compile` clean on titles.py + main.py; find_duplicates harness 7/7 PASS (venv + real `init_db` schema — never hand-rolled, per the v0.70.0 lesson); frozen six untouched (git-verified); recon settled an Open_Questions verify-flag with no code change: **MarkFinishedModal.jsx imports ui/StarRating** (:5, rendered :64).
- **New visible strings flagged verbatim for ratification** (full variant list in the session report): sums sentence `{X} read(s) and {Y} note(s) move over.` · trash clause `{F} file(s) go(es) to the trash folder.` · `Merged {M} of {N}, then failed` · help strip `Select the title to keep, then tap "Merge into selected". Other titles will be merged into it.` · empty state `Your library is clean.` · `View title details` · `{n} titles` (×2). **Ratified in the pre-commit round (2026-07-25):** validation error ships as `Select a title to keep` (no "Please "); trigger label ships sentence-case `Merge into selected` (both occurrences). Reused v0.84.0-ratified strings NOT re-flagged: parts vocabulary, `no file on disk`, `Merged — {…} moved over`, `Merge` / `Keep separate`. **Made calls flagged:** row counts render zeros (triage comparison beats suppression; the confirm sentence still suppresses); `has_file === false` strict check (a stale backend renders no false ghost indicators); failure detail console-only behind the ratified numbered string; `Merged 0 of {N}, then failed` is reachable when the first merge fails.
- **Backend files changed (titles.py + version): full Docker rebuild required. No schema change, no migration, no bulk writes — no `library.db` backup needed (subqueries and a read-only stat pass only).**

## [0.84.0] — 2026-07-25

**Merge-confirm redesign, session 1 of 2 — the BookDetail surface. Implements the eight Decisions of 2026-07-25: the confirm step stops describing a static operation and describes the computed outcome. Keeping-first framing (kept title leads, action-primary tint; SOURCE/TARGET all-caps labels, the red/green boxes, the arrow SVG, and the static "Moves over"/"Goes to the trash folder" lists are all deleted), real per-category counts with zero rows suppressed, a trash line only when files will actually move, a metadata-honesty caption, and post-merge feedback from the response body both surfaces previously discarded. DuplicatesPage is session 2 (v0.85.0) — untouched here.**

### Added
- **Select-time target fetch.** Picking a merge target now fires one `getBook(target.id)` (existing endpoint, no new backend surface — Decisions 2026-07-25). The Keeping card's meta line (file count + cover type) renders from it, with a quiet `Loading…` caption while pending. A stale-response guard (version ref, the codebase's race idiom) keeps a slow fetch for a previous pick from clobbering a newer one, and `openMergeModal` invalidates in-flight fetches from a prior open. **On fetch failure the confirm renders from TitleSummary data with no meta line — the merge is never blocked on the preview** (per the session prompt; the one sanctioned degrade-not-surface path, logged here rather than silent).
- **Post-merge feedback via the shared `ui/Toast`** (already mounted in BookDetail — no new primitive). `handleMerge` stops discarding the merge response: the success toast reports what actually moved (`merged.sessions/notes/collections`, zero counts suppressed) and the conditional carries (`cover_carried`, `wishlist_note_converted` — knowable only post-merge, never predicted at confirm, Decisions 2026-07-25). Shown before `navigate()`; `/book/:id` is one route, so the param-change navigation keeps the component mounted and the toast survives. Toast duration 5000ms (default is 3000ms — longer read for a composed message; made call, flagged below).
- **ComponentPreview section 16: SortDropdown** (rider owed from v0.83.0). Controlled demo with Library's option list, contract caption (required `options`, default-direction-on-new-field via the exported helper, tap-active-to-flip, bespoke rows by decision). Import↔export grep both sides: default import ↔ `export default function SortDropdown` (ui/SortDropdown.jsx:55). **Arithmetic correction to the owed line:** the gallery is now **16 of 17** demoed, not seventeen of seventeen — recon found `SettingsRow` (rehomed to `ui/` in v0.82.0 as a byte-identical move) never had a gallery section either. That gap predates this session and is flagged for its own rider, not improvised here.

### Changed
- **Confirm step describes the computed outcome** (content model ratified 2026-07-25; root cause of the mid-test scare: static copy asserting "This can't be undone" semantics contradicted the v0.55–v0.71 lossless records-operation reality):
  - **Keeping card first** — `bg-action-primary/15 border border-action-primary` (the codebase's established tint pair), label `Keeping`, title + author, meta line from the select-time fetch: file count (`editionFileOnDisk` idiom per edition — `file_path` present AND `file_size` not null, so stale paths never inflate the count) + cover type (`custom cover` / `extracted cover` / `generated cover`; bare `cover` if `has_cover` with no source).
  - **Merging-away card second** — neutral `bg-bg-base border border-border-default`, label `Merging away`, meta line from page state: file count + read count; a zero-file source shows `no file on disk`.
  - **"What happens" list, computed, zero rows suppressed:** sessions+notes sentence (combined when both nonzero, verb agreement by total), `Joins N collections`, and the trash line **only when the source has on-disk files**: `N file(s) move(s) to the trash folder — recoverable until you empty it`. When every row is zero the whole block (heading included) suppresses.
  - **Metadata-honesty caption always renders:** `The kept title's details win — this one's tags and metadata aren't carried.`
  - The old static intro card ("This title's reading history, notes, and collections move to the one you picked. Its files move to the trash folder…") is deleted with the rest of the static copy — it asserted unconditional file-trashing that is false for fileless sources, exactly what the ratified model retires.
  - Footer unchanged: `Merge` (danger) / `Keep separate` / `← Back`.
- **Source-side counts read existing page state** (`sessions`, `notes`, `bookCollections` — the Delete Title modal precedent, Decisions 2026-07-25). Recon verified none can be empty-by-timing: notes resolve inside the page-gating `Promise.all`, sessions/collections are eager fetches launched at page load (never lazy-on-open), and the counts re-render live if state lands late.
- **Search-step source-card caption is now conditional** to match the trash logic: `This title — its history moves over, its files go to trash` only when on-disk files exist; otherwise `This title — its history moves over`. Other search-step strings untouched.
- **"of reading history" retired, both occurrences (pre-commit ratification round, 2026-07-25):** the merge confirm's sessions+notes row ships trimmed (`{R} reads and {N} notes move to the kept title`, verb-by-total logic unchanged), and the ratification reached the phrase's one other occurrence — the Delete Title modal's v0.55.0 consequence sentence (`…leaves your library, along with {R} reads and {N} notes`), the precedent the merge row was built from. Repo-wide grep now 0.
- **`api.js` `mergeTitles` JSDoc corrected to v0.56.0+ semantics** (rider, Decisions 2026-07-25): editions never move to the target — sessions/notes/collections/links move, source files go to trash, target keeps its metadata and exactly its own files, cover carries only when it wins, non-empty `tbr_reason` converts to a note. The old comment ("Moves all editions… from source to target") had described the pre-v0.56.0 operation for fourteen versions.
- **Pre-token utilities → token classes in touched blocks only** (rider): the confirm step and the search-step source card now use `text-h4`/`text-body-sm`/`text-caption` + config color utilities (`text-h4` = 1rem/500 — pixel-identical to the replaced `font-medium` at base size; `text-body-sm` ≡ `text-sm`). Search-result rows and all other untouched blocks keep their existing classes (Golden Rule 2).

### Removed
- **From the confirm step, per the ratified mockup:** `SOURCE (will be deleted)` / `TARGET (will be kept)` all-caps labels, the danger/success tinted boxes, the arrow SVG, the static `Moves over` list (Reading sessions / Notes / Collection memberships / Backlinks), the static `Goes to the trash folder` list, and the `Files can come back from the trash folder. Emptying it is manual.` caption (superseded by the inline recoverable-clause on the computed trash line). Grep-verified zero live-tree matches for `SOURCE (will be deleted)`, `TARGET (will be kept)`, `Moves over` (only hits: a stale `.claude/worktrees/` checkout of an older commit — session tooling, not the repo tree).

### Technical
- **Modified:** `frontend/src/components/BookDetail.jsx` (merge state +3 — `mergeTargetDetail`, `mergeTargetLoading`, `mergeTargetFetchIdRef`; `openMergeModal`/`selectMergeTarget`/`handleMerge`; merge-facts block beside the Delete Title facts; search-caption conditional; confirm-step rewrite), `frontend/src/api.js` (JSDoc only), `frontend/src/pages/ComponentPreview.jsx` (SortDropdown import + demo state + section 16), `backend/main.py` (0.83.0 → 0.84.0), `docs/DESIGN_LINT_REPORT.md` (regenerated — BookDetail marker line-shifts only), `CHANGELOG.md`, `ROADMAP.md`.
- **Verification:** design-lint FULL strict all ten categories 0/pass, chrome inventory unchanged at 38 (no raw buttons added or removed), zero ignore lines; esbuild parse exit-0 on BookDetail.jsx, ComponentPreview.jsx, api.js; `py_compile` clean on main.py; `mergeTitles` callers unchanged (api.js definition + BookDetail + DuplicatesPage — DuplicatesPage zero-diff); frozen six untouched; `backend/routers/titles.py` zero-diff (session-2 file).
- **New visible strings flagged verbatim for ratification** (no silent copy invention — full variant list in the session report): `Keeping` · `Merging away` · `What happens` · `Loading…` · meta-line vocabulary (`N file(s)` / `no file on disk` / `N read(s)` / `custom cover` / `extracted cover` / `generated cover` / `cover`) · `{parts} move(s) to the kept title` · `Joins N collection(s)` · `N file(s) move(s) to the trash folder — recoverable until you empty it` · `The kept title's details win — this one's tags and metadata aren't carried.` · search-step no-file variant `This title — its history moves over` · toast strings `Merged` / `Merged — {…} moved over` / `cover carried` / `wishlist note carried` · toast duration 5000ms (made call).
...
### Fixed
- **Priority popup (wishlist BookDetail) renders correctly again.** Recon confirmed the mechanism: the popup is absolutely positioned inside a containing block only as wide as the trigger chip, so shrink-to-fit clamped it to `min-w-[140px]` — and S4's MenuItems (base text + px-4, ≈147px for "⭐ High Priority") wrapped where the old 14px/px-3 rows (~126px) had fit. Container-level fix: `w-max` on the popup (sizes to its widest item; the 140px floor stays). **MenuItem itself is untouched — no STOP fired.**
- **The stranded green ✓: hypothesis KILLED — it is not selected-state residue.** It's the `priorityStatus` save-confirmation (`'saved'` → ✓ 2s, `'error'` → `!` 3s, both auto-clearing via `handlePriorityChange`), shipped before the adoption sprint and untouched by S4; the error twin is the only inline failure surface for priority saves, so deletion would have violated no-silent-failures. Per the STOP condition: kept, placement fixed — the wrapper becomes `relative flex items-center`, making chip and indicator flex siblings that can't wrap under each other when the row squeezes. No ratification line for the indicator was found in the Decisions blocks read; flagged for the record, kept on the merits.

### Changed
- **Raw-`<button>` is STRICT** (report-only since S13, ratchet-held since C recon). The matcher now classifies every occurrence outside `ui/`/frozen; the implemented test, precisely: a site is EXEMPT iff **(1)** its opening tag carries `role="option"`, or **(2)** it is structurally a content surface — zero bare-text direct children (every non-whitespace character sits inside an element subtree or a `{…}` expression), at least one direct child element that is not icon/label anatomy (`svg`, `span`, and `/Icon$/`-named component tags never qualify), no nested `<button>` at any depth **in either direction** (a button inside a button — the SortDropdown defect — never exempts, wrapper or wrapped), and a well-formed close. `{…}` expression children are permitted and neutral. Anything still counted must carry `design-lint-button-chrome: <annotation>` in a comment on its line or the line above — **the annotation is required; a bare marker suppresses nothing.** Markers suppress only this category (unlike `design-lint-ignore`) and are inventoried in DESIGN_LINT_REPORT.md's new Chrome-marker section.
  - **Two matcher tightenings beyond the ratified two criteria, per the tighten-and-report STOP:** (a) the both-directions nested-button clause — the first draft exempted SortDropdown's chevron sort-direction buttons (icon-component children), hiding half the parked invalid-HTML defect; (b) the `/Icon$/` component rule — uppercase glyph components (SortIcon, ChevronUpIcon, DragHandleIcon) defeated the svg/span carve-out and structurally exempted the SortDropdown trigger and CollectionCard drag handle, neither a content surface. With both, SortDropdown counts exactly the ratified 9.
  - **The delta-0 ratchet had no separate machinery to delete** — it lived in the `strict: false` flag, the report's "report-only until the backlog clears" footnote, and a session-report verification convention. The flag and footnote flipped/rewrote with the category; nothing else existed.
- **47 chrome markers placed** (full inventory in DESIGN_LINT_REPORT.md): 9× `provisional pending SortDropdown restructure` (the parked defect, post-sprint queue, restructure ahead of merge-confirm); 1× `ratified-bespoke (v0.60.0 status toggle)` + 1× `ratified-bespoke (session form semantic actives)` (the S4-STOPPED segment groups, reclassified 2026-07-23 as ratified bespoke controls); 33 plain chrome (10 active-filter chips + 8 view toggles + 3 mobile tabs + 3 acquisition tabs + 2 filter pill rows + 2 collection-type selectors + 2 chip-removes flagged as IconButton-shaped post-sprint candidates + priority-popup trigger + drag handle + banner dismiss); and 3 honesty annotations on sites below the structural bar but not chrome in spirit — BookDetail's format-picker rows (option-shaped), UploadSuccess's span-composed result row (re-anatomize post-sprint), and CollectionPicker's create-slot (parked NOT-B1 2026-07-22; its qualifying child was a `*Icon`, so the tightened rule un-exempted it — the park's "likely B3a rescope" prediction lands as a marker instead).
- **`role="option"` added to six option rows** (the ratified five text-only — WishlistForm series suggestions, AuthorChips suggestions, TagsMultiSelect tag list, UnifiedEditModal's two autocomplete lists — plus WishlistForm's title-suggestion row, which is element-childed and dual-covered by both criteria; all six are genuine combobox options, so the a11y correction applies to the full population). Container `role="listbox"` wrappers were not ratified and are noted as the remaining half of the combobox pattern.
- **`ui/ThreeDotMenu` trigger tap target 32px → 44px** (`p-1.5` → `p-3` around the 20px glyph) — one fix, three surfaces inherit (BookDetail, CollectionDetail, CollectionsTab). Sheet Cancel's raw `text-base` → `text-body` token (same 16px). Both were the v0.81.0 reviewer findings, ratified as S4b tidy-ups.
- **DuplicatesPage match-type pill → Badge (the eighth site, ratified):** "Exact Match"/"Similar Title" now `variant="tint"` `tone={danger|warning}` sm square beside the already-adopted Same Author pill; `danger` added to Badge's tint table (the anticipated one line). Same `/15`→`/20`+border normalization as its neighbor.
- **`SettingsRow` moved `components/settings/` → `components/ui/`** (already a shared primitive, wrong folder — 2026-07-23 ruling). Plain filesystem `mv` + the single import update in Settings.jsx; **git stays read-only per CLAUDE.md — the rename lands through Marie's `git add .` (rename detection at commit)**. Its two internal raw buttons (navigation row, toggle switch) become `ui/`-internal and leave the count. Zero stale-path references (repo-wide grep). DESIGN_SYSTEM §3: count line now sixteen, SettingsRow entry added.
- **BookLinkPopup close ✕ → `IconButton size="sm"` with `aria-label="Close"`** (ratified — IconButton-shaped, real a11y gap; the svg glyph rides along, `type="button"` per §3). **New microcopy flagged verbatim for ratification: `"Close"`.**

### Technical
- **Modified:** `scripts/design-lint.mjs` (classifyButton + tagEnd, chrome-marker mechanics, strict flip, report sections), 15 marker hosts (`BookDetail`, `Library`, `SortDropdown`, `AuthorDetail`, `AuthorChips`, `TagsMultiSelect`, `CollectionDetail`, `CollectionsTab`, `CollectionCard`, `CollectionModal`, `CollectionPicker`, `DuplicateCollectionModal`, `FilterDrawer`, `add/ManualEntryForm`, `upload/UploadSuccess`), role additions (`add/WishlistForm`, `UnifiedEditModal` — AuthorChips/TagsMultiSelect double as marker hosts), `BookLinkPopup.jsx`, `ui/ThreeDotMenu.jsx`, `ui/Badge.jsx`, `pages/Settings.jsx` (import path), `pages/DuplicatesPage.jsx`, `docs/DESIGN_SYSTEM.md`, `docs/DESIGN_LINT_REPORT.md` (regenerated — Chrome-marker inventory section is new), `backend/main.py` (0.81.0 → 0.82.0), CHANGELOG.md, ROADMAP.md. **Moved:** `frontend/src/components/settings/SettingsRow.jsx` → `frontend/src/components/ui/SettingsRow.jsx`. **Backend file changed (version bump only): full Docker rebuild required. No schema change, no migration, no bulk writes — no `library.db` backup needed.**
- **Marker-count arithmetic, every unit attributed:** 70 (v0.81.0) − 2 (SettingsRow → ui/) − 1 (BookLinkPopup ✕ conversion) = 67 lint-visible; 67 = **47 marked + 20 structurally exempt + 0 unmarked**. The 20: 6 role="option" rows + 14 element-children content surfaces (BookDetail stat cards ×2, BookLinkPopup result row, CollectionCard list card, CollectionModal cover-type rows, CollectionPicker collection rows, Fandom/Ship/Tags modal selection rows ×3, FilterDrawer escalation rows ×3, SearchModal result row, ManualEntryForm title suggestions). Priority-popup and rider fixes moved no counts (the trigger's marker was placed, not removed).
- **Verified:** design-lint FULL run **all ten categories 0/pass, exit 0, strict raw-button included**; ignore inventory "None"; chrome inventory 47/47 annotated (zero MISSING); esbuild parse clean on all 23 touched JSX files (exit-code verified); `py_compile` clean; SettingsRow import↔export both sides with zero stale paths; frozen six untouched (`upload/BookCard.jsx`'s 2 buttons remain frozen-excluded, not exempt-counted); nothing deleted this session — the ✓ survives by design, so no deletion sweeps were owed.
- **Code-reviewer pass (frozen / scope / conformance): 3/3 PASS**, including an independent synthetic probe of the strict classifier on an isolated copy — 4/4 planted defects flagged (unmarked bare-text button, annotation-less marker, both halves of a nested pair) with both structural exemptions honored; all 47 marker insertions verified comment-only; SettingsRow verified byte-identical across the move. One low finding fixed in-session (§3 Badge tint enumeration gained `danger`); one advisory carried to the post-sprint queue: SettingsRow's pre-existing `bg-white` toggle knob — a default-palette literal now living in `ui/`, invisible to lint (A1 matches `text-white` only), correctly not fixed during a byte-identical move.

## [0.81.0] — 2026-07-24

**Adoption sprint S4 — the build session. The MenuItem primitive ships, all 15 B3b menu-item sites convert across six containers, ThreeDotMenu finally extracts to `ui/` (ROADMAP 10.0.14's checked-but-false claim becomes true, with CollectionDetail and CollectionsTab adopting it), Badge is rebuilt on tokens and adopted at all 7 census sites, and the SegmentedControl/StarRating orphans gain production consumers. Raw-`<button>` 95 → 70. Two SegmentedControl targets STOPPED on census mismatch (per-status active colors the component cannot express) — reported, not improvised. The strict flip is S4b, not this session.**

### Added
- **`ui/MenuItem.jsx` — the menu-row primitive** (contract locked 2026-07-22, visual elected 2026-07-23). Renders `<button type="button">` — or a react-router `Link` when `to` is passed — as `w-full flex items-center gap-3 px-4 py-3`, rest `text-text-primary`, `danger` → `text-action-danger`, `disabled` dims (40%, inert), optional leading icon slot (16px, collapses when absent), hover `bg-bg-surface`, no rounding (the container clips). Props: `children, icon, onClick, to, danger, disabled, className` (layout-only, Button's contract verbatim). Deliberately excluded per the ratified "When NOT to use": trailing slot, submenus, selected state, roving focus. Rows clear 44px at base text size (py-3 + 24px line box = 48px).
- **`ui/ThreeDotMenu.jsx` — extracted from BookDetail** (was inline at :147–:278 since before v0.65.0). Keeps the controlled `menuOpen`/`setMenuOpen` + data-driven `menuItems` API (`label/onClick/danger/show/divider`) and the v0.65.0 portaled-sheet fix verbatim; items now render via MenuItem in **both** the desktop dropdown and the mobile bottom sheet. **One API addition, flagged not silent: items accept `icon`,** passed through to MenuItem's slot — required the moment CollectionDetail adopted (its five rows have icons; the MenuItem icon slot was elected for exactly these menus — S1 recon: "icons in 2 of 5"). Sheet chrome (handle, Cancel, backdrop) stays in the component.
- **ComponentPreview sections 14 + 15** — MenuItem (default / icon / danger / disabled / to, in a mock elevated container) and a live controlled ThreeDotMenu (divider, danger, and a `show: false` row demonstrated). Badge section rewritten to the new API showing the census shapes.
- **DESIGN_SYSTEM §3 entries** for MenuItem (with the ratified exclusions written as "When NOT to use") and ThreeDotMenu (portaled sheet recorded as frozen behavior); §3 count line now "Fifteen components."

### Changed
- **Containers standardized on `bg-bg-elevated`** (ratified 2026-07-23, resolving the five-site hover split): BookDetail's desktop dropdown and mobile sheet move `bg-bg-surface` → `bg-bg-elevated`; CollectionDetail/CollectionsTab/CollectionCard/BookContextMenu already sat on elevated. MenuItem's `bg-bg-surface` hover is now the one hover value everywhere. Knock-on chrome edits inside the extracted sheet, forced by the container change: handle `bg-bg-elevated` → `bg-border-default` (SortDropdown's existing handle treatment — same-color-as-sheet would vanish), Cancel hover `bg-bg-elevated` → `bg-bg-surface`, sheet items now full-bleed (the px-2 inset + per-item rounding died with MenuItem's no-rounding election; dividers unify to the desktop hairline).
- **All 15 B3b menu-item sites render via MenuItem** — the full denominator (13 drift-check + 2 reclassified): BookDetail ThreeDotMenu desktop + sheet maps (2, via extraction), CollectionDetail (5, via adoption), CollectionsTab (2, via adoption), CollectionCard context menu (2, direct), BookContextMenu (2, direct — inline SVGs became icon props), BookDetail priority-popup options (2, direct — the 2026-07-23 B4→B3b reclassification). `stopPropagation` (CollectionCard), close-on-select, `show` conditionals, and danger flags preserved verbatim. Elected deaths, as ratified: BookDetail's `text-text-body` outlier and raw `text-sm`/`text-base` item sizes; **the priority options' selected-state tint** (teal/amber on the current choice) also dies — MenuItem excludes selected state by decision, and the trigger chip already displays the current value.
- **CollectionDetail + CollectionsTab adopt ThreeDotMenu**, replacing hand-rolled triggers + dropdowns (backdrop divs included). Both menus **gain the portaled mobile bottom sheet** (previously plain dropdowns at all widths) and both triggers **gain `aria-label="More actions"`** via the shared trigger — flagged below for microcopy ratification. Trigger glyph standardizes on BookDetail's filled-dots svg; CollectionsTab items grow px-4 py-2 → px-4 py-3 (44px+ rows), CollectionCard items likewise. Their local `DotsIcon` components deleted as dead (grep-verified, both files).
- **`ui/Badge` rebuilt from scratch on tokens** — the old file's 7 inline-style hexes are gone; class strings are full literals in tone tables. API (proposed from the 7-site census, exactly at the ≤4-variant cap): `variant` **solid** (filled semantic color: success/primary/danger/warning — warning takes `text-text-inverse`, the rest `text-text-primary`) / **tint** (`{tone}/20` fill + tone text + `{tone}/30` border: warning/fiction/fanfiction/nonfiction/fandom/character) / **outline** (bg-base + default border + text-body) / **muted** (`bg-bg-elevated/80` + text-muted); `size` sm/md/lg (caption/caption/body-sm — the raw `text-xs`/`text-sm` at census sites map pixel-identically to `text-caption`/`text-body-sm`); `pill` (rounded-full default, `false` → rounded); `title` passthrough; `className` layout-only. Unknown variant/tone renders null (old convention kept). **No status mode** — no census site renders reading-status labels; §3 records that a future status mode must route through `useStatusLabels`.
- **Badge adopted at all 7 census sites:** WishlistTab priority overlay (solid warning sm, absolute positioning via layout className), UploadSuccess result pills (solid success/primary/danger sm square), ChangeCoverModal source pill (conditional tint fanfiction/character or muted md), BookDetail edition format badges (tint md, tones from FORMAT_CONFIG; unknown formats fall back to muted) + TBR category chip (outline lg), CollectionDetail type badges (muted sm square ×2), DuplicatesPage Same Author (tint fanfiction sm square).
- **`constants/formats.js` FORMAT_CONFIG `color` class-strings → `tone` fields** — the color strings existed solely for the BookDetail badge site (grep-proven single consumer at :1589/:1594); the format→display mapping stays in the config, the styling moves into Badge's tone tables. `EBOOK_CHIP` const deleted with them.
- **Settings' two segment groups + WishlistTab's priority filter adopt `ui/SegmentedControl`** (production shape: `size="sm"` + `ariaLabel`; WishlistTab was recon-gated and fit without contortion — its wrapper/active classes were already SegmentedControl's verbatim). Settings actives change register: teal `bg-action-primary` fill → the component's neutral elevated pill, labels 14px → 11px; WishlistTab labels 14px → 11px, segments equal-width, `aria-pressed` gained. Both flagged for the phone pass.
- **MarkFinishedModal's hand-rolled star picker → `ui/StarRating`** (interactive, `size="lg"`): 28px SVG stars in 36px targets become 30px glyphs in 44px targets with hover fill and per-star aria-labels ("Rate N of 5" — shipped StarRating strings reaching a new surface). Toggle-to-clear maps `null` → the modal's 0 state; the S2-converted ghost Clear at 36px stays beside it. `hoveredRating` state deleted (decl + setter grep-verified). **This modal is now twice-touched since its S2 phone flag — phone pass required.**

### Fixed
- **ROADMAP 10.0.14 is true at last** — the "Record corrected 2026-07-17" note (checked-but-never-extracted) resolves: `ui/ThreeDotMenu.jsx` exists, BookDetail imports it, and the menu has three consumers instead of one inline single-consumer copy.
- **The DESIGN_SYSTEM §3 Badge entry's stale ImportPage citation is gone** (ImportPage was deleted v0.62.0; the census correction was recorded 2026-07-22 and the fix rode this rewrite, as ratified).

### STOPPED — census mismatch, reported not improvised
- **BookDetail's reading-status toggle (:2067 at drift-check numbering, :1932 post-session) and session-status segments (:3015 → :2880) did NOT adopt SegmentedControl.** The prompt's premise — "production shape: size=sm + ariaLabel," i.e. existing props suffice — does not hold at either site: both use **per-status active colors** (`STATUS_TOGGLE_ACTIVE`'s B2-ratified "reading teal, finished sage" status tokens; the session form's success/chip-ship/secondary actives with `useStatusLabels` labels), the reading toggle carries a `disabled={statusSaving}` double-tap guard and the ratified (2026-07-15) content-driven 2×2 grid flip for long custom labels — all inexpressible in SegmentedControl's `value/onChange/options/size/ariaLabel` API without new props, which is the census-mismatch STOP condition. Adopting would have silently reverted two ratified decisions and dropped a race guard. Both sites untouched; each keeps its one raw-`<button>` occurrence (accounted in the arithmetic below). Disposition — S4b chrome markers or a SegmentedControl variant extension — is Marie's call.

### Technical
- **Created:** `frontend/src/components/ui/MenuItem.jsx`, `frontend/src/components/ui/ThreeDotMenu.jsx`.
- **Modified:** `frontend/src/components/BookDetail.jsx` (inline ThreeDotMenu deleted −133 lines, `createPortal` import dropped with it grep-verified, priority options + format badges + category chip converted), `frontend/src/components/CollectionDetail.jsx` (menu adoption, DotsIcon deleted, type badges), `frontend/src/components/CollectionsTab.jsx` (menu adoption, DotsIcon deleted), `frontend/src/components/CollectionCard.jsx`, `frontend/src/components/BookContextMenu.jsx`, `frontend/src/components/WishlistTab.jsx` (PriorityBadge deleted, filter adoption), `frontend/src/components/upload/UploadSuccess.jsx` (getBadge → RESULT_BADGES map), `frontend/src/components/ChangeCoverModal.jsx` (getCoverSourceClasses deleted), `frontend/src/components/MarkFinishedModal.jsx`, `frontend/src/components/ui/Badge.jsx` (full rebuild), `frontend/src/constants/formats.js`, `frontend/src/pages/Settings.jsx`, `frontend/src/pages/DuplicatesPage.jsx`, `frontend/src/pages/ComponentPreview.jsx`, `docs/DESIGN_SYSTEM.md`, `docs/DESIGN_LINT_REPORT.md` (regenerated), `backend/main.py` (version 0.80.0 → 0.81.0), CHANGELOG.md, ROADMAP.md. **Backend file changed (version bump only): full Docker rebuild required. No schema change, no migration, no bulk writes — no `library.db` backup needed.**
- **Deleted implementations, all grep-verified to zero:** BookDetail inline ThreeDotMenu, `PriorityBadge`, `getBadge`, `getCoverSourceClasses`, `EBOOK_CHIP` + `FORMAT_CONFIG.color`, both `DotsIcon`s, `hoveredRating`/`setHoveredRating`, and the old Badge internals (`STATUS_BACKEND_KEY`/`STATUS_CONFIG`/`CATEGORY_CONFIG`/`CHIP_CONFIG`).
- **Raw-`<button>` arithmetic, occurrence-level, expected vs actual — every unit attributed:** from **95**: −2 B3b item maps via extraction (BookDetail desktop + sheet), −2 extraction chrome (trigger + Cancel, now in `ui/`), −2 dots-adoption triggers (CollectionDetail :1005, CollectionsTab :313 — call-site `<button>`s replaced by ThreeDotMenu's internal trigger, counted honestly as the prompt required), −5 CollectionDetail item rows + −2 CollectionsTab item rows (B3b sites cleared by the same adoption; the prompt's minus-list named only the triggers — these seven are the adopted menus' item rows from the 15-site denominator), −2 CollectionCard, −2 BookContextMenu, −2 priority options, −3 WishlistTab segments (recon gate passed), −2 Settings segment maps + −1 MarkFinishedModal star map (the incidental B4/B3a deaths via SegmentedControl/StarRating adoption). **Total −25 → 70, matching the lint run exactly.** The two STOPPED BookDetail segment maps keep +2 that full adoption would have removed (68 was the everything-adopts number).
- **Verified:** esbuild parse clean on all 16 touched JS/JSX files (exit-code checked); design-lint (deno) **all nine strict categories 0/pass, zero ignore lines**, raw-button 70 as above; **no new `!` overrides** (the six `!justify-start` hits in BookDetail are pre-existing S2/S3-era Modal/Button layout classes, untouched); no new raw `<button>` outside `ui/`; import↔export grep both sides for every added import (MenuItem/ThreeDotMenu/Badge/SegmentedControl/StarRating/FORMAT_CONFIG — all default/named exports printed and matched); `py_compile` clean on backend/main.py; frozen six untouched (git status).
- **New/changed microcopy flagged verbatim for ratification:** `aria-label="More actions"` (pre-existing BookDetail string, now on all three ThreeDotMenu triggers via adoption — two new surfaces), `ariaLabel="Filter by priority"` (new, WishlistTab), `ariaLabel="Books per row (mobile)"` and `ariaLabel="Card style"` (new as aria, Settings — mirror their visible row labels).
- **Code-reviewer pass (frozen / scope / conformance): 3/3 PASS**, two minor findings — both pre-existing styling carried verbatim through the extraction, report-and-decide for S4b: the shared trigger's ~32px tap target (p-1.5 + w-5 glyph, now on three adopters), and the sheet Cancel's raw `text-base` (chrome, not an item; invisible to cascade-flip lint). Neither fixed — unratified chrome edits are exactly what Golden Rule 2 exists to stop.
- **Observed, not converted (census boundary respected):** DuplicatesPage :220's match-type pill ("Exact Match"/"Similar Title") is the same display-pill shape one line above the adopted Same Author but was not among the 7 census sites — left hand-rolled, offered for ratification (would be `tint` sm square with a danger/warning tone; danger needs one line in Badge's tint table). ChangeCoverModal's tinted pills gained the tint variant's `/30` border and the gradient pill went `bg-bg-elevated` → `/80` (variant normalization); DuplicatesPage Same Author `/15` → `/20` fill + border; WishlistTab's overlay grew 10px → 12px text — all flagged for the phone pass.
...
### Changed
- **17 raw `<button>` sites converted to `ui/Button`** (DESIGN_SYSTEM §3 contract: system variants only, `className` layout-only, zero `!` overrides):
  - *primary* — App.jsx fatal-error "Retry" (sole action of the dead-app screen); AcquireCard "I got this one" (`className="flex-1"` layout retained). Both were already `bg-action-primary`-styled — zero-drift mappings.
  - *secondary* — CollectionDetail remove-mode "Cancel" (cancel-adjacent action in the selection bar).
  - *danger* — CollectionDetail "Remove {n}" (batch remove; `disabled` when nothing selected, logic preserved verbatim); CollectionModal "Remove custom cover" (destructive, was danger-text-styled); CriteriaBuilder "Clear all" (destroys all built rules; its old `text-action-danger` already signaled the register).
  - *ghost* — the quiet-action family, all previously link-styled text actions (teal/underline/caption — actions dressed as links, the Button-vs-Link violation this category existed to catch): BookDetail TBR "Edit" + "+ Add a reason why", BookLinkPopup `Insert "…" as plain text →`, CollectionModal + DuplicateCollectionModal "Info"/"Hide" disclosure toggles (`InfoIcon` moved into Button's `icon` slot), MarkFinishedModal rating "Clear", SearchModal `Filter library by "…" →`, StatusLabelsModal "Reset" (aria-label preserved; `-mr-5` layout class replicates the old `px-3`/`-mr-3` right-flush), AddPage error-banner "Dismiss" (the action is benign — the danger tint belonged to the banner, not the act), DuplicatesPage "Try again" + "Rescan" (refresh svg → `icon` slot; `disabled={loading}` preserved — no spinner existed, so no `loading` prop invented).
  - Sizes: `md` wherever the old target was ≥44px or the button is its surface's main action (both primaries, the action-bar pair, SearchModal, StatusLabelsModal, Rescan — 7 sites); `sm` for the visibly compact inline family (10 sites). Several tiny text actions gain real tap targets (MarkFinishedModal "Clear" was ~20px).
  - `type="button"` retained on every site that had it (Button doesn't inject one; inside forms the native default is submit).

### Fixed
- **B1 census corrected: 17, not the triage's 19.** Root cause: the 2026-07-20 B-triage ran against the pre-C1 124-site lint data; C1 (v0.73.0) then deleted AcquireCard's dormant block — which contained exactly two text-labeled action buttons ("Mark Finished", "Status ▾") — and taught the lint to strip comments. Commit archaeology (`be623ad`) shows both dead B1 sites; no `<button` line has entered or left `frontend/src` since; TruncatedText (C1's other deletion) lived in `components/ui/` and was never in the census. The block's other two buttons were icon-only — S3's B2 pool inherits the same recount caveat.

### Technical
- **Modified:** `frontend/src/App.jsx`, `components/AcquireCard.jsx`, `components/BookDetail.jsx`, `components/BookLinkPopup.jsx`, `components/CollectionDetail.jsx`, `components/CollectionModal.jsx`, `components/CriteriaBuilder.jsx`, `components/DuplicateCollectionModal.jsx`, `components/MarkFinishedModal.jsx`, `components/SearchModal.jsx`, `components/settings/StatusLabelsModal.jsx`, `pages/AddPage.jsx`, `pages/DuplicatesPage.jsx` (17 sites, 13 files; 6 files gained the `Button` import — export grep-verified both sides), `backend/main.py` (version 0.76.0 → 0.77.0), `docs/ARCHITECTURE.md` (gallery routing bullet unstaled — the z-order-collision claim died in v0.76.0), `README.md` (version restamp 0.75.0 → 0.77.0), `docs/DESIGN_LINT_REPORT.md` (regenerated), CHANGELOG.md, ROADMAP.md. **Backend file changed (version bump only): full Docker rebuild required. No schema change, no migration, no file operations, no bulk writes — no `library.db` backup needed.**
- **Verified:** esbuild parse clean on all 13 touched JSX files; `py_compile` clean on backend/main.py; design-lint (deno) all nine strict categories 0/pass, raw-`<button>` **120 − 17 = 103**; no `!` overrides added in any touched file (S12 Category-C guarded — the working diff's only `!` classes are v0.76.0's ratified demo-frame fix in ComponentPreview.jsx, untouched this session); BookDetail's four A6 `stroke="#5c5752"` active-ignores still count exactly 4 (B2 territory, S3's); no state added or deleted — every identifier in converted JSX pre-existed at its site (Button import/usage scorecard 13/13).

## [0.76.0] — 2026-07-22

**S1b rider — the gallery exit ships. Executes the three S1 Ratifications (Decisions.md 2026-07-22) verbatim; S1 is now shipped in full across v0.75.0 + v0.76.0.**

### Added
- **`/dev/components` has a real exit** — a production UnifiedNavBar (back link "Settings" → `/settings`) mounted at the top of the gallery, above the content container. The route stays outside ConnectedApp (unchanged, by decision). Back label follows the destination-naming convention (CollectionDetail's backLabel="Collections" precedent). The R5 phone-verify is still pending and rides this deploy — the Settings link below ships in the same build, so one mobile test covers the pair.
- **Settings gains its first developer-facing surface** — new bottom grouping **"Developer"** after Trash, one row, **"Component gallery"**, linking to `/dev/components` through SettingsRow's `to` prop (renders a `<Link>`; adds no raw button). Copy ratified 2026-07-22, used verbatim. Gate honored: the row was added only after both gallery edits parsed clean.

### Fixed
- **The S1 z-order collision is dead.** Root cause (v0.75.0 recon): the demo frames de-stickied their UnifiedNavBar specimens (`!relative !top-auto`) but left `z-20` intact — equal z-index with the real bar meant the later-DOM demos painted over it while scrolling underneath. The ratified one-liner lands: both demo-frame wrappers now also force `[&>div]:!z-0`, so the specimens keep their frames and lose only the stacking win. Verified two ways: (1) mechanism repro in a browser — a wrapper rule `z-index: 0 !important` against the specimen's own z-20 class computes to 0, and at the exact overlap geometry where the S1 probe returned the demo element, `document.elementFromPoint` now returns the real bar, which renders complete and opaque with the demo label clipped beneath it; (2) reasoning against the fixed CSS — real bar (z-20) vs demo stacking contexts (z-0) in the same root stacking context: the higher z-index paints above regardless of DOM order, so the later-DOM tiebreak that caused the S1 collision no longer applies.

### Technical
- **Modified:** `frontend/src/pages/ComponentPreview.jsx` (the two wrapper class strings + the UnifiedNavBar mount with a two-line placement comment), `frontend/src/pages/Settings.jsx` (Developer grouping: SectionHeader + one SettingsRow), `backend/main.py` (version 0.75.0 → 0.76.0), CHANGELOG.md, ROADMAP.md. **Backend file changed (version bump only): full Docker rebuild required. No schema change, no migration, no file operations, no bulk writes — no `library.db` backup needed.**
- **Verified:** esbuild parse clean ×2 (ComponentPreview.jsx before the gated Settings edit — honoring the build-clean gate — then Settings.jsx); `py_compile` clean on backend/main.py; design-lint (deno) all nine strict categories 0/pass, **raw-`<button>` 120, delta 0** (tool printed "Report unchanged" — the Settings row is a Link, the navbar mount consumes a `ui/` component). No new imports anywhere (UnifiedNavBar was already imported in ComponentPreview, SettingsRow already in Settings — cross-module export rule not triggered); no new state or identifiers.

## [0.75.0] — 2026-07-22

**Adoption Sprint S1 — gallery + recon. Partial ship: demos, recon verdicts, and doc riders landed; the gallery exit (and the Settings link hard-gated behind it) STOPPED on a verified z-order collision.**

### Added
- **Component gallery: the four missing demos** — `/dev/components` now demos every `ui/` component with a §3 entry. AuthorInput (interactive string value + empty + boolean-error rows, caption noting the on-mount author fetch), ChipInput (interactive array value with static suggestions + string-error row; its own embedded FormField, per contract), SegmentedControl (the production shape only — `size="sm"` + `ariaLabel` inside `FormField label="Category"`, options mirroring ManualEntryForm's category picker; the locked 11px sm label noted in the caption), FileDropZone (controlled files + disabled row; static constraints with a caption stating production fetches them from `/api/upload/limits`). All demo state is display-only `useState` — nothing persists, nothing uploads. Sections appended as 10–13 after UnifiedNavBar; existing sections untouched, no renumbering.
- **FormField's Rating slot is real now** — the dashed `[StarRating would go here]` placeholder (the §3 children-mode example that didn't practice what it documented) is an interactive `StarRating size="md"` on its own display-only state. The prompt's "StarRating placeholder slot" item found a full StarRating section already shipped (interactive sm/md/lg with a clearable row + read-only sm/md/lg — already satisfying the interactive-md/read-only-sm/toggle-to-clear spec); the FormField placeholder was the only placeholder actually left, so that is what was filled (made-call, flagged below).
- **DESIGN_SYSTEM.md §4: the error-register split lands** (ratified 2026-07-19, unlanded until now). Empty/loading/error states now define two registers split by who acted: LOAD failures (passive — fetch failed under the user) render quiet `text-text-secondary` inline text + Try again inside the failed region; ACTION failures (user acted, it didn't take) render the danger banner at the point of action. The v0.63.0 inline-banner precedent explicitly governs the second register only.
- **ROADMAP: the adoption sprint gets its public table** — S1–S4 scope/status rows (none existed; the plan lived only in the decision log), placed before Search Expansion, which already declared itself queued behind this sprint. S1 marked partial with the STOP named. Current State header → v0.75.0; Current Focus's "Next up" clause now reads "Now underway … S1 shipped in v0.75.0" (one-clause tense edit to the ratified batch-close paragraph — flagged for ratification).

### Reported — stop conditions honored (nothing built)
- **Gallery exit + header (and the gated Settings link) STOPPED on a verified navbar/demo-frame collision.** The plan: a real UnifiedNavBar (back affordance → `/settings`) on `/dev/components`, demo navbars staying in their de-stickied frames. The blocker: UnifiedNavBar's root is `sticky top-0 z-20`; the two demo frames force `!relative !top-auto` on it but leave `z-20` intact — so the real bar and both demo specimens sit at z-20 in the root stacking context, and at equal z-index later-DOM paints on top. Scrolling the UnifiedNavBar section under the pinned real bar makes the demo frame paint OVER it. Verified empirically, not just on paper: a minimal repro of the exact CSS (sticky z-20 first in DOM vs de-stickied relative z-20 later) in a browser shows the demo bar sliding over the pinned bar, and `document.elementFromPoint` in the overlap strip returns the demo element. Candidate one-line fix for ratification: extend the two demo-frame wrappers (ComponentPreview :501/:506 pre-session) with `[&>div]:!z-0` — demos stay inside their frames, lose only the stacking win; alternatives (raising the real bar's z) all touch UnifiedNavBar itself, whose look is deliberately not overridable. Per the ruling, the exit was not shipped and no fix was improvised; the Settings row (SettingsRow `to` prop, verified v0.73.0) stays parked with its R5 gate intact.
- **R1 Badge verdict: SURVIVES to the S4 rebuild — count 7, nowhere near the delete branch.** Census of non-frozen adoptable badge-like display pills (hand-rolled status/category/chip label pills a rebuilt Badge could render): WishlistTab :11–:18 (PriorityBadge "High"), upload/UploadSuccess :56–:67 + :148–:151 (NEW/+FORMAT/ERROR result pills), ChangeCoverModal :127–:131 (cover-source pill on chip tokens), BookDetail :1718–:1733 (edition format badges via FORMAT_CONFIG), BookDetail :1873–:1877 (read-only category chip in the TBR block), CollectionDetail :1136–:1145 (greyscale Checklist/Auto type badges), DuplicatesPage :229–:233 ("Same Author" pill). Excluded as a different component class, not counted: interactive dismissible/toggle chips (Library's nine active-filter pill groups with embedded remove buttons, TagsMultiSelect chips, FilterDrawer toggle pills, BookDetail's priority-popup trigger); numeric/icon cover overlays (SeriesCard count, AuthorDetail #N, SearchBar/Library count bubbles, BookCard status-icon overlays); frozen upload/BookCard.jsx :169 (excluded by ruling). **Census correction for the record: the decision's known-site list named ImportPage — deleted in v0.62.0, zero references in frontend/src; only two of the three known sites still exist.** Badge.jsx and its gallery demo untouched, per the ≥2 branch.
- **R2 MenuItem five-site markup dump captured for the S4 visual spec** (full verbatim JSX in the session report): BookDetail ThreeDotMenu desktop :214–:220 + mobile sheet :250–:256 (data-driven items, `danger` flag, dividers, no icons, no `type="button"`, raw text-sm/text-base sizes), CollectionCard ContextMenu :121–:145 (py-2 rows, no icons), CollectionDetail in-page menu :1020–:1111 (five items WITH leading icon components, py-3, mixed `type="button"`), CollectionsTab in-page menu :329–:345 (py-2, no icons), BookContextMenu :28–:51 (inline SVG icons, py-3). Spread confirmed: three row paddings (py-2 / py-3 / py-3.5), icons in two of five, hover on `bg-bg-surface` vs `bg-bg-elevated` split by menu background, danger via conditional class in two shapes.
- **R3 doc accuracy, now that the gallery was to become reachable:** ARCHITECTURE.md :223 carried two route-list inaccuracies fixed this session (dead `/import` still listed — ImportPage deleted v0.62.0; live `/sync-results` missing) and one claim that stays false while the STOP holds — "Every routed page renders UnifiedNavBar" — now scoped honestly to ConnectedApp with the gallery's missing exit named as the blocked S1 item. FRONTEND_AUDIT_S12.md :189 (route-wiring ✅ for `/dev/components`) stays true — the route and component are unchanged; its "(dev-only route)" label and :55's typography census (18 ImportPage usages; 9 ComponentPreview) are S12-snapshot history — reported, not edited, per the audit-is-historical rule.

### Documentation
- **README:** the refresh rider arrived already satisfied — caa2d2b ("docs: README refresh to v0.74.0", the prior session) had landed version/date, SmartPaste removal, and Cursor→Claude Code; verified this session (SmartPaste 0 hits, Cursor 0 hits). Only the version line moved: 0.74.0 → 0.75.0. No public-voice sentences added.
- ARCHITECTURE.md routing bullet corrected (see R3 above). DESIGN_SYSTEM.md §4 split (see Added). Stale mention noted for S4, not edited: §3 Badge's "Common mistakes" still cites ImportPage among local badge implementations.

### Technical
- **Modified:** `frontend/src/pages/ComponentPreview.jsx` (4 imports, `DEMO_CATEGORY_OPTIONS` const, 5 state hooks, Rating-slot fill, sections 10–13), `backend/main.py` (version 0.74.0 → 0.75.0), `docs/DESIGN_SYSTEM.md` (§4 register split), `docs/ARCHITECTURE.md` (routing bullet), `README.md` (version line), CHANGELOG.md, ROADMAP.md. **Deleted: nothing.** **Backend file changed (version bump only): full Docker rebuild required. No schema change, no migration, no file operations, no bulk writes — no `library.db` backup needed.**
- **Verified:** esbuild parse clean on ComponentPreview.jsx (native darwin-arm64 binary via deno cache); `py_compile` clean on backend/main.py. Cross-module export grep, both sides, for all four new imports: `AuthorInput.jsx:4` / `ChipInput.jsx:8` / `FileDropZone.jsx:23` / `SegmentedControl.jsx:1` — each `export default function <Name>` ↔ one new ComponentPreview import. Design-lint (deno): **all nine strict categories 0/pass; raw-`<button>` 120, delta 0** — the tool itself printed "Report unchanged" (new demo sections add zero raw buttons; ChipInput/FileDropZone internals live in `ui/`, exempt by rule). New identifier pairing (grep line counts, decl line included): `starField` 2 / `setStarField` 2, `authorsDemo` 3 / `setAuthorsDemo` 2, `chipsDemo` 3 / `setChipsDemo` 2, `segmentDemo` 3 / `setSegmentDemo` 2, `filesDemo` 3 / `setFilesDemo` 2, `DEMO_CATEGORY_OPTIONS` 2 — every declaration has at least one render use and every setter is wired; no unbound identifiers.
- **Microcopy flags for ratification:** gallery demo strings are dev-facing and follow the existing gallery register, but for the record: LabelRow captions in sections 10–13, demo placeholder "Add a tag...", demo error strings "Must have at least one tag" (mirrors the shipped WishlistForm register) and the existing gallery-precedent style. **Proposed for the parked Settings row (NOT shipped): grouping header "Developer", row label "Component gallery"** — Settings has no developer-ish grouping today (Appearance/Reading/Library Tools/Backups/Trash), so the grouping itself is also copy to ratify.
- **Made-calls needing ratification:** (1) "StarRating placeholder slot" executed as the FormField Rating-slot fill — the prompt's premise (no StarRating section) was stale; the existing section already exceeds the asked demo, and the dashed placeholder was the only placeholder left. (2) New demos appended as sections 10–13 rather than inserted among the input components — zero churn on existing sections. (3) `DEMO_CATEGORY_OPTIONS` mirrors ManualEntryForm's options minus Uncategorized (three segments read better in a demo; production's four-option shape named in the §3-mirroring caption). (4) ARCHITECTURE.md UnifiedNavBar sentence scoped to ConnectedApp with the gallery exception named — the pre-session sentence was already false for `/dev/components`, and the STOP keeps it false, so the honest form names the blocker. (5) ROADMAP sprint table created (prompt said "mark S1 in the sprint table"; no table existed) + the one-clause tense edit to the ratified Current Focus paragraph. (6) README version line stamped 0.75.0 on top of the prior session's completed refresh.



## [0.74.0] — 2026-07-20

**Batch 3 C2 — the batch closer (docs errata + microcopy pass + hex rescope). Batch 3 closes at this version: v0.62.0–v0.74.0.**

### Changed
- **Ratified copy corrections (2026-07-20), implemented verbatim — E1–E6:**
  - **E1 AcquireCard:** `🎉 I Got This Book!` → **"I got this one"** — exclamation + emoji + "Book" predated the voice pass; the last stale-copy queue item from v0.73.0 closes. No aria text existed on the button (recon-verified): the visible label is the accessible name, so there was no separate aria string to update. The upload chooser's "Move to library" is a different surface in a deliberately different register — untouched.
  - **E2 trash toast:** `Trash emptied.` → **"Trash emptied"** (toast rule: past tense, no terminal period — the one v0.66.0 string that broke it).
  - **E3 downloads.py 404 detail:** `File not found — it may have moved since the last library scan` → **"Couldn't find the file — it may have moved since the last scan."** — the backend detail now matches the string MICROCOPY_LIBRARY already approved (File moved/missing row). R2 verified the frontend *renders* this detail as a passthrough (BookDetail :1418 `message = data.detail`) and never compares on it; SyncResultsPage :274's similar hint is a distinct plural literal, untouched.
  - **E4 Replace-file confirm (BookDetail :2475):** `…never changes this book's details.` → **"…never changes this title's details."** (inclusive terminology; source uses `&apos;` — rendered string matches the ratified form).
  - **E5 duplicate refusal (upload.py :559/:1185 — same verbatim string, both refusal paths: review route + link route):** `…use Replace file in the book's Files section…` → **"…in the title's Files section…"** at both sites.
  - **E6 upload result strings sentence-cased, first letter only, all 7 sites:** **"Same-format duplicates kept but not recorded: "** (×3: :625/:973/:1018), **"Already recorded, kept the existing edition: "** (×2: :621/:1013), **"Edition records deferred — run a full sync after the format migration succeeds"** (×2: :617/:1008, the second wrapped across two literals — first literal's first letter cased).

### Added
- **design-lint A6 — the unbracketed-hex rescope ships (C1 item-8 stop, resolved by the 2026-07-20 ratified scope).** The rule the v0.73.0 stop demanded: unbracketed hex is lint-caught where it is *styling* — className string values, SVG `stroke=`/`fill=` attributes, and CSS declarations — and stays code-reviewer judgment where it is *data* (inline-style objects, generation-data constants: CollectionGradient lanes, Badge chip colors, SeriesCard fallbacks). Exact-length alternation (8|6|4|3) keeps hex-shaped words from matching; A4 still owns the bracketed form. index.css's token-mirror block (4 hexes, base styles + scrollbar pseudo-elements that can't take utilities) sits between a new **`design-lint-hex-sanctioned-start`/`-end`** comment pair — CSS-only, read from unstripped source like ignore markers, exempting A6 ONLY (every other category still applies inside) — with the one-line why on the start marker. **H3 gate honored: the scoped matcher was run BEFORE any ignore was written and found exactly the four expected sites — BookDetail :1962/:2321/:2380/:2537 `stroke="#5c5752"` — and nothing else.** Each now carries a `design-lint-ignore` comment naming the value, the reason (raw icon-button glyph predates the shared IconButton), and cleared-by (shared-component adoption sprint — B2 IconButton conversions); all four are individually listed in the report's active-ignores inventory, so the category strict-passes because every exception is written down, not because the matcher is blind. Category label `Hardcoded colors (A1–A5)` → `(A1–A6)`.

### Documentation (batch-close errata — D1–D5 + the approved-strings update)
- **MICROCOPY_LIBRARY.md** absorbs every microcopy flag from v0.63.0–v0.72.0 (reconciliation printed in the session report): Error Strings gains the v0.63.0 collection/cover set + the v0.72.0 load-failure set; new **Diagnostic register (per-item result rows)** subsection carries the v0.67.0/v0.68.0 rejection vocabulary + the E6-corrected result strings + the E3-corrected detail, with the system-fixture carve-out note; **Empty trash** (v0.66.0) and **Replace file** (v0.70.0) recorded as complete suites with the E2/E4/E5 corrections; new **Wishlist → Library & Upload Results** section (v0.69.0 banner set, "Move to library", "I got this one", v0.71.0 outcome headers, prefix v2 current with v0.70.0's superseded); StatusLabelsModal Reset strings (v0.65.0) split to their homes (visible link under Status Labels, aria in Screen Reader Labels); "Trash emptied" + "File replaced" join the toast list.
- **DESIGN_SYSTEM.md §4 Status labels (D1):** the v0.65.0 StatusLabelsModal exception recorded — field labels render static canonical defaults, never live-updating custom values; per-field Reset renders only when value ≠ default; reviews should stop re-flagging it.
- **DESIGN_SYSTEM.md §5 (D2 + H4):** alert() row's "14 legacy call sites" text killed — the ledger closed at v0.63.0 with 0 sites, strict since the v0.57.0 rider. Hardcoded-colors row now states the A6 scope: className/stroke/fill/CSS lint-caught since v0.74.0, inline-style + generation data are judgment; non-listed hues remain the honest CHANGELOG 0.50.0 gap.
- **FRONTEND_AUDIT_S12.md (D3):** dated rename notes appended at all five ReadingStatusCard / CollapsibleSection references (→ AcquireCard v0.72.0; → TruncatedText v0.72.0, deleted v0.73.0). Counts and findings untouched — the audit is a baseline snapshot; the §11 methodology regex is preserved byte-exact with the note outside the backticks.
- **ROADMAP.md (D4 + D5):** Current Focus replaced with the ratified batch-close paragraph (public-safety verified: no internal IPs/NAS/home-network details); Current State header v0.62.0 → v0.74.0 with two added rows — upload write-path containment (v0.67.0–v0.70.0) and lossless wishlist → library conversion (v0.69.0–v0.71.0).

### Technical
- **Modified:** `frontend/src/components/AcquireCard.jsx` (E1 button copy), `frontend/src/pages/Settings.jsx` (E2 toast), `frontend/src/components/BookDetail.jsx` (E4 confirm copy + four A6 ignore comments), `frontend/src/index.css` (sanction marker pair), `scripts/design-lint.mjs` (A6 patterns + CSS branch + className hex check + category label + header docs), `backend/routers/downloads.py` (E3 detail), `backend/routers/upload.py` (E5 ×2 + E6 ×7), `backend/main.py` (0.73.0 → 0.74.0), `docs/MICROCOPY_LIBRARY.md`, `docs/DESIGN_SYSTEM.md`, `docs/FRONTEND_AUDIT_S12.md`, `docs/DESIGN_LINT_REPORT.md` (regen: A6 label + four active-ignores), CHANGELOG.md, ROADMAP.md. **Backend files changed: full Docker rebuild required. No schema change, no file operations, no bulk writes — no `library.db` backup needed.**
- **Verified:** `py_compile` clean ×3 (downloads.py, upload.py, main.py); esbuild parse clean ×3 (AcquireCard, Settings, BookDetail); design-lint (deno) **all nine strict categories 0/pass — including A6 — with the four ignores individually inventoried; raw-`<button>` 120, delta 0** (the four inserted comment lines shift report line numbers only). **No identifiers added or deleted in `frontend/src` or `backend/`** — every product-code edit is a string literal, JSX text, or comment; the lint script (config, not product code) gained four module consts (`UNBRACKETED_HEX` 1 decl + 3 uses, `SVG_HEX_ATTR` 1+1, `HEX_SANCTION_START` 1+1, `HEX_SANCTION_END` 1+1) and branch locals (`sanctioned` 1+2, `openLine` 1+3, `hexHits` 1+2), pairing-counted. No imports added or removed anywhere (cross-module rule not triggered). E1–E6 grep scorecard: new forms 2/3/2/2 where multi-site, old forms 0 repo-wide in code.
- **Made-calls needing ratification:** (1) A6 designation + category-label rename A1–A5 → A1–A6 (v0.73.0's label-rename precedent: the report stays honest with the matcher). (2) The ratified "sanctioned marker" implemented as a start/end PAIR — a single marker can't bound a block; the start line carries the one-line why. (3) E5 applied at both occurrence sites — the prompt named one string; it lives verbatim in two refusal paths. (4) E6's seven sites for three strings, same reasoning. (5) MICROCOPY organization: suites recorded complete-in-place as Confirmations & Buttons subsections; Diagnostic register under Error Strings; new top-level Wishlist → Library & Upload Results section; "Trash emptied"/"File replaced" double-listed (toast rule registry + their suites); E3's string listed in the Diagnostic register with a cross-reference to the pre-existing Error Strings row rather than duplicated as a new approval. (6) ROADMAP Current Focus kept the `> **Current Focus:**` blockquote label; body is the ratified text verbatim. (7) Current State's two new rows chosen as upload containment + wishlist conversion (the shipped batch-3 systems most missing; the inline-error work is carried by the Current Focus paragraph instead of a third row).

## [0.73.0] — 2026-07-20

### Removed
- **`ui/TruncatedText.jsx` deleted (Batch 3 C1; gallery-demo-is-not-a-consumer ratified 2026-07-20).** Zero production importers: BookDetail's usages died in v0.58.0's kill-collapse session, leaving ComponentPreview's gallery demo as the only reference — and a demo is not a consumer. ComponentPreview loses the import, the `longText` demo body, and the whole TruncatedText `<Section>` (both demo instances and their stale body titles die with it); the UnifiedNavBar section comment renumbers 10 → 9. `docs/DESIGN_SYSTEM.md` loses the §3 entry — which still carried the *CollapsibleSection* title the v0.72.0 rename never reached — and the §3 intro's "Fourteen components." becomes "Thirteen" (made false by this deletion; every other DESIGN_SYSTEM erratum stays for C2). **Riders that die with the file:** the C-pass lint item targeting its `text-teal-400` toggle (nothing left to fix) and the `rgb(26, 26, 31)` cool-fade bug candidate (CHANGELOG 0.50.0). Golden-rule-4 sweep printed in-session: `TruncatedText` **0** and `CollapsibleSection` **0** in `frontend/src` AND in `docs/DESIGN_SYSTEM.md`; historical CHANGELOG/ROADMAP + `docs/FRONTEND_AUDIT_S12.md` mentions stay (history; audit errata are C2's).
- **AcquireCard's dormant block deleted (the v0.72.0 dead-code candidate, now the session that names it).** Dead since v0.60.0 evicted the owned-book render; the only caller is BookDetail's `isWishlist`-gated card passing `status="unread"` / `subtitle` / `isWishlist` / `onAcquire` (call site reconfirmed verbatim). Deleted: props `fileUrl`, `hasFile`, `onEditSession`, `onMarkFinished`, `onChangeStatus`; icons `DownloadIcon`, `EditIcon`; handlers `handleDownload`, `handleEdit`; the `showDownload`/`showEdit` flags on every `STATUS_CONFIG` entry; derived `showDownloadAction`; and the download / edit / Mark Finished / Status ▾ button JSX (`EditIcon`/`handleEdit`/`showEdit` are reachable only through the dormant `onEditSession` path — included as made-call, flagged for ratification below). The live wishlist render is behavior-identical: same config lookup, same label/subtitle markup, same acquire-button block byte-for-byte (`🎉 I Got This Book!` untouched — its copy is C2's). `status`, `STATUS_CONFIG`, `CheckIcon`, and `displayLabel` survive: `status` is a real prop with live variants even though today's caller pins it.

### Added
- **`IconButton` gains `variant="muted"`** — third variant beside default/accent: rest color `text-text-muted`, hover `hover:bg-bg-elevated hover:text-text-primary`, exactly what the `!` important overrides produced on top of default. Both R2 sites (BookDetail Files rows — Replace file, Remove ×) convert to it and their `!text-text-muted hover:!text-text-primary` overrides are deleted; zero `!` overrides remain on any IconButton usage. §3 IconButton entry updated (variants line); recon found **no** doc note sanctioning the override workaround, so there was nothing to drop. Non-muted `!` sites found by R2's wider sweep are NOT IconButton and were left: BookDetail :3180/:2802/:2813/:2839/:2865/:2891 (`Button`/`Modal.Footer` layout overrides), ComponentPreview :527/:532 (demo-frame de-sticky wrappers), Button.jsx :44 (internal loading opacity).
- **StatusLabelsModal: the failed-label field takes the danger border** alongside its existing `role="alert"` message, per §4 Forms (danger border + message at the field). Border keyed to the same `saveError?.key === key` state as the message (BookDetail :3087's conditional-border pattern), so it clears exactly when the error clears — next persist attempt, successful Reset, or modal reopen.

### Changed
- **design-lint strips comments before matching (S13 counting-rule deviation, corrected baseline).** Root cause of a baseline that has been wrong since S13: the audit's "counts include comments/JSDoc" rule let `RAW_BUTTON` count the `<button` inside SettingsRow.jsx:21's JSDoc (`renders a <Link> instead of <button>`) — the raw-`<button>` baseline was 125 when the true count was **124**. New `stripComments` is offset-preserving (comment bodies become spaces, newlines kept — every reported line number identical), string/template-literal aware (`https://` in strings and `${}` nesting survive), CSS strips block comments only (`//` inside `url()` is not a comment), and `design-lint-ignore` markers are still read from the original, unstripped source. Documented limitation: an unescaped `//` inside a regex literal reads as a line comment (escaped `\/\/` handled via backslash guard) — same family as the pre-existing brace-scanner regex caveat. 16/16 edge-case harness (scratchpad, deno). Header counting-rules comment updated to record the deviation from FRONTEND_AUDIT_S12 §0.
- **The window.confirm rule now matches bare `confirm(`** — same function, different spelling; the rule was evadable from the day it was written. New pattern `/(?<![\w.$])(?:window\.)?confirm\(/g`: bare and `window.`-qualified match once each; `showConfirm(`/`dialog.confirm(`/`myconfirm(` do not (8/8 behavior checks printed). Category label now "window.confirm() / bare confirm()"; §5 anti-pattern row's enforcement text updated to match (the row's stale `alert()` clause left for C2). R4 recon: **zero** `confirm(` call sites of any spelling in `frontend/src` — pure hardening, count stays 0.

### Reported — stop conditions honored (nothing built)
- **Item 5 (Settings row → `/dev/components`) STOPPED on R5: no working mobile exit.** Evidence: App.jsx :96 mounts ComponentPreview OUTSIDE ConnectedApp — no BottomNav; the page's own header is a plain `h1` with no nav chrome; the only UnifiedNavBar instances are *demo specimens* inside the gallery's final section — one title-only (no back affordance at all), one whose `backTo="/"` link works mechanically but sits below ~9 sections of scroll, labeled "Sample Page", inside a demo frame that deliberately defeats its sticky positioning. On a 380px viewport in the installed PWA (no browser chrome), that is a dead end, not an affordance. The row was not shipped; SettingsRow's `to` prop was verified anyway (renders a `<Link>`, adds no raw `<button>`) for whichever session ships the exit + row together.
- **Item 8 (unbracketed-hex gap) STOPPED: the tightened rule finds 32 sites — over the 15-site stop threshold.** Full list in the session report; summary: index.css :11/:20/:24/:29 (4 — sanctioned token-mirror base styles), CollectionGradient.jsx :15–:24 (10 — gradient-lane *generation data*, same family as frozen BookCard's constants), SeriesCard.jsx :27/:28/:29/:89/:100 (6 — cover-fallback data + inline-style surfaces), BookDetail.jsx :1962/:2321/:2380/:2537 (4 — the known `stroke="#5c5752"` SVGs, shifted from the prompt's :1955/:2314/:2374/:2531 by interim sessions), Badge.jsx :33–:68 (8 — chip text colors as inline-style data). All inside `frontend/src`; none is a direct token swap in a named file, and most are *data-not-className* values a naive hex rule mis-frames — the rule needs a ratified scope (className/stroke/fill contexts vs. generation-data files) before 28 ignores get written. No matcher change shipped: shipping it un-ignored would flunk strict, and a strict category must pass because exceptions are written down, never because the matcher is blind — today the matcher is still blind to unbracketed hex, and §5's known-gaps clause (unchanged) still says so honestly.

### Technical
- **Deleted:** `frontend/src/components/ui/TruncatedText.jsx` (must also be removed from the Docker volume by hand). **Modified:** `frontend/src/pages/ComponentPreview.jsx` (import, longText, TruncatedText section, comment renumber), `frontend/src/components/AcquireCard.jsx` (dormant block), `frontend/src/components/ui/IconButton.jsx` (muted variant + JSDoc), `frontend/src/components/BookDetail.jsx` (two IconButton conversions only), `frontend/src/components/settings/StatusLabelsModal.jsx` (danger border), `scripts/design-lint.mjs` (comment strip + confirm matcher + header comments), `backend/main.py` (0.72.0 → 0.73.0), `docs/DESIGN_SYSTEM.md` (§3 CollapsibleSection entry removed, "Thirteen components.", IconButton variants line, §5 confirm enforcement text — all made false by this session's changes; all other errata left for C2), `docs/DESIGN_LINT_REPORT.md` (lint regen), CHANGELOG.md, ROADMAP.md. **Backend file changed (version bump only): full Docker rebuild required. No schema change, no file operations, no `library.db` backup needed.**
- **Verified:** esbuild parse clean ×5 (ComponentPreview, AcquireCard, IconButton, BookDetail, StatusLabelsModal; deno npm:esbuild); `py_compile` clean on `backend/main.py`. Item-1 sweep: `TruncatedText` 0 / `CollapsibleSection` 0 (frontend/src + DESIGN_SYSTEM.md). Item-2 sweep, case variants included: all 12 deleted identifiers 0 in `frontend/src`; every remaining substring hit is a *different* identifier alive in its own file with decl/use pairing (BookContextMenu's own `onMarkFinished`/`onChangeStatus` props + its three callers' handlers, BookDetail `showDownloadSheet` 3/`setShowDownloadSheet` 2 · `handleDownloadEdition` 3 · `handleDownloadClick` 2 · `openEditSession` 4, FileDropZone/AddToLibrary `hasFiles`, CollectionDetail/CollectionsTab `showEditModal`/`setShowEditModal` pairs · `handleEditCollection` 5 · `handleEditSuccess` 2, IconButton JSDoc's `handleEdit` example). No React state deleted → setter-case rule vacuously satisfied, variants grepped anyway. No new imports added (cross-module rule not triggered; IconButton pairing printed regardless: export :16 ↔ BookDetail import 1). Design-lint (deno): **all nine strict categories 0/pass; raw-`<button>` 125 → 124 (comment strip, exactly SettingsRow:21) → 120 (AcquireCard :124/:135/:149/:160 deleted; the surviving acquire button re-lines at :88)** — hard arithmetic check passed with zero unexplained delta; report diff shows exactly those five rows gone and the label rename, nothing else.
- **Microcopy flags for C2:** no strings added or changed this session. Two stale-copy queue items *close as moot*: ComponentPreview's demo body titles "Open Section (long body)" / "Collapsed Section (long body)" died with the TruncatedText demo. Still queued and untouched: `🎉 I Got This Book!`.
- **Made-calls needing ratification:** (1) `EditIcon`/`handleEdit`/`showEdit` deleted under R1's "reachable ONLY from the dormant paths" clause though not in its example list — the edit button's sole effect was `onEditSession?.()`, and the arithmetic check's expected raw-button lines (:135 among the four) confirm the prompt intended its deletion. (2) window-confirm category *label* renamed in the report table (rule text said update §5; the label rename keeps the report honest with the widened matcher). (3) "Fourteen components." → "Thirteen" in the §3 intro — read as made-false-by-this-session under the DOC SCOPE RULE.

## [0.72.0] — 2026-07-19

### Changed
- **`ReadingStatusCard` renamed `AcquireCard` (ratified 2026-07-17).** Recon confirmed the expected wishlist-only usage: v0.60.0 evicted the owned-book render, so the sole remaining usage is BookDetail's wishlist-gated Acquire card (passing only `status="unread"`, `subtitle`, `isWishlist`, `onAcquire`). File + component + BookDetail import/JSX renamed; the component-file diff is naming-only (two lines: doc comment + function name), verified by byte diff against the old file before deletion. **Reported, not deleted:** the dormant download affordance rides along whole — `fileUrl`/`hasFile` props, per-status `showDownload` flags, `DownloadIcon`, `handleDownload`, and the download button are unreachable from the one live render (reserved for S16 in v0.51.0; S16 closed without claiming it), and `onEditSession`/`onMarkFinished`/`onChangeStatus` + their button blocks are equally dormant — a dead-code candidate for a session that names it. Also observed for the microcopy ledger, untouched: the Acquire button copy `🎉 I Got This Book!` (exclamation + "Book" + emoji) predates the voice pass.
- **`ui/CollapsibleSection` renamed `ui/TruncatedText` (ratified 2026-07-17).** The name has lied since it was built (v0.57.0 implementation note): it truncates content behind a gradient fade with a View more toggle — it never collapsed a section (header and partial content always render). Sole remaining importer was ComponentPreview (BookDetail's usages died in v0.58.0 — confirmed by recon); its import, gallery comment, `<Section title>`, and four JSX tags renamed. The component-file diff is naming-only (two lines), byte-diff-verified. `text-teal-400` (line 96) deliberately persists — **the C-pass lint item now targets `TruncatedText.jsx`**. ComponentPreview's demo body titles ("Open Section (long body)" / "Collapsed Section (long body)") are content, not the symbol — left as-is, flagged as stale-copy candidates for the C-pass.

### Fixed
- **List-view silent-failure audit: every blocking console-only catch across the eight named surfaces now renders where the failed action lives** (v0.63.0 banner precedent + the 2026-07-17 banner-placement rule; `role="alert"` on each new surface). Shared root cause: catch blocks whose only action was `console.*` — invisible on mobile, where ~95% of usage lives and no console exists. Converted sites, each with state + render + clear-on-retry/next-action:
  - **CollectionPicker `loadCollections`** — a failed load left `collections` empty, so the picker rendered the *lying* "No collections yet / Create your first collection below." empty state, inviting duplicate collections. Load failure now replaces the body with a banner + Try again (rerunning `loadCollections`); the empty state is unreachable on error.
  - **CollectionPicker `toggleCollection`** (ledgered) — a failed add/remove left the checkbox unmoved with zero explanation. Picker-body banner: "Couldn't save your changes. Try again?" (approved string); the retry is tapping the row again, which clears the banner.
  - **CollectionPicker `handleNewCollectionSuccess`** — create-then-add failed silently: the new collection appeared, unchecked, with the add quietly dropped. The banner states both truths and the remedy: "Created the collection, but couldn't add this title to it. Tap it below to try again."
  - **StatusLabelsModal load** — a failed `getSettings` silently rendered the four DEFAULTS as if they were current; a blur from that wrong baseline persists a value the user never chose over a custom they couldn't see. Modal-top banner + Try again (load extracted to `loadLabels` so retry exists); banners reset on modal reopen.
  - **StatusLabelsModal `persistLabel`** (ledgered v0.65.0) — the save-on-blur/Reset write failed invisibly, and the v0.65.0 reviewer's sharpened Reset case held: the optimistic set flipped the Reset link's render condition false, so a failed Reset showed the default, hid the link, and left the DB on the custom label. The catch now **rolls back the optimistic value** — the field and the Reset link render what the database still holds (the reappeared link IS the retry affordance) — and reports "Couldn't save your changes. Try again?" (approved string) at the failed field's row, the point of action in a modal with no Save button. Cleared on the next persist attempt and on reopen. Rollback is per-key (`prev => ({ ...prev, [key]: prevValue })`), so a concurrent edit of another field is never clobbered.
  - **CollectionDetail load-more ×3** (`loadMoreBooks`, `loadMoreIncomplete`, `loadMoreCompleted`) — a failed infinite-scroll page silently truncated the list: the IntersectionObserver doesn't refire while the sentinel stays visible, so the list just ended with `hasMore` still true. Each sentinel row now renders "Couldn't load more titles." + Try again wired to that section's loader. Per-section state (`'main' | 'incomplete' | 'completed'`); the main path sets the error under the same sort-version guard as its success path (a stale request's failure can't paint an error over a fresh sort); cleared on that section's next attempt and on every `fetchCollection` (sort change, navigation).
  - **CollectionDetail + CollectionsTab drag-reorder** — a failed order save reverted silently: the row snapped back with no explanation (a rejection-only path with no explicit state, golden rule 3). Both surfaces render "Couldn't save the new order. Try again?" directly under the reorder-mode banner; cleared on the next drag attempt and on entering reorder mode fresh. CollectionsTab's existing revert-by-refetch and CollectionDetail's local revert are unchanged — only the silence is gone.
  - **Benign, left silent (justifications):** localStorage read/write guards (Library :28/:36/:116, AuthorDetail :17/:25/:34/:68/:74, WishlistTab :101) — preference persistence in storage-less environments; the user's action itself succeeds. WPM/reading-speed prefetches (Library :192, AuthorDetail :109, CollectionDetail :578) — read-time estimates fall back to the numeric default; decoration-level accuracy only. **Library :181 `getCategories`** — failure degrades the FilterDrawer's category options; an honest surface would thread error state into `FilterDrawer.jsx`, outside the named file set → **parked** per the audit's stop rule (and the primary book-load path already errors loudly with its own retry).
  - Not audit members (already surfaced): Library/AuthorDetail/WishlistTab/SeriesDetail page loads, every B2 status-action handler, CollectionsTab/CollectionDetail fetch + delete + batch-remove catches — all set rendered error state.
- **SortDropdown duplicate bottom sheet: stacking verified sound on every rendering page — no code change (Item 4, Branch B).** All four rendering surfaces walked ancestor-by-ancestor to `<main>`: Library :552 (Browse toolbar — a sibling *after* the sticky z-30 tab bar, not inside it), AuthorDetail :402, CollectionDetail :1142 (automatic collections only, rendered after `</UnifiedNavBar>`), WishlistTab :250 (inside Library's plain content div). No ancestor creates a stacking context — no position+z-index, transform, filter, backdrop-filter, or will-change (grep-verified across all four files; the only hits are leaf elements inside cards: dnd-kit row transforms, a series badge's `backdrop-blur-sm`), and SortDropdown's own `relative` wrapper carries no z-index. The in-tree `fixed` sheet (z-50) and backdrop (z-40) therefore join the ROOT stacking context: the sheet stacks above the z-40 BottomNav and, being bottom-anchored with handle + label + options + Cancel, always covers the nav's 64px strip completely — the nav is neither visible nor tappable while the sheet is open; the backdrop dims the sticky tab bar (z-30) and UnifiedNavBar (z-20). No coexisting-overlay scenario exists: CollectionDetail's remove-mode bar and reorder mode are `!isAutomatic`-gated while its SortDropdown is `isAutomatic`-gated (mutually exclusive), and modals/menus can't open while the sheet is up (any tap outside it lands on the backdrop and closes it). The v0.65.0 ThreeDotMenu bug does not apply here because that sheet rendered *inside* UnifiedNavBar's sticky z-20 stacking context — SortDropdown's sites have no such ancestor. Desktop variant (`absolute` z-50 dropdown) wins over the sticky bars by the same root-context logic, and BottomNav is `md:hidden`. Recorded so the "duplicate impl" note stops resurfacing as a stacking suspicion; **the duplicate implementation itself (SortDropdown's in-tree sheet vs ThreeDotMenu's portaled sheet) remains a dedup candidate** for a session that names it — not acted on, per the v0.65.0 rider that portaling requires its own containment-ref surgery.

### Technical
- **Added:** `frontend/src/components/AcquireCard.jsx` (was `ReadingStatusCard.jsx`), `frontend/src/components/ui/TruncatedText.jsx` (was `ui/CollapsibleSection.jsx`). **Deleted:** `frontend/src/components/ReadingStatusCard.jsx`, `frontend/src/components/ui/CollapsibleSection.jsx`. **Modified:** `frontend/src/components/BookDetail.jsx` (import + JSX tag only), `frontend/src/pages/ComponentPreview.jsx` (import + gallery references), `frontend/src/components/CollectionPicker.jsx`, `frontend/src/components/settings/StatusLabelsModal.jsx`, `frontend/src/components/CollectionDetail.jsx`, `frontend/src/components/CollectionsTab.jsx` (audit surfaces), `backend/main.py` (0.71.0 → 0.72.0), `docs/DESIGN_LINT_REPORT.md` (lint regen — rename churn + line shifts only), CHANGELOG.md, ROADMAP.md. **Backend file changed (version bump only): full Docker rebuild required. No schema change, no file operations, no `library.db` backup needed.**
- **Verified:** esbuild parse clean ×8 (every touched/renamed JSX file, native darwin-arm64). Rename scorecards: `ReadingStatusCard` **0 matches** and `CollapsibleSection` **0 matches** in `frontend/src`; `AcquireCard` pairs export 1 ↔ import 1 + JSX 1 (BookDetail), `TruncatedText` pairs export 1 ↔ import 1 + JSX 2 open/2 close (ComponentPreview). Cross-module import rule, both sides printed in-session: `AcquireCard.jsx:70` / `TruncatedText.jsx:19` `export default function …` ↔ their single importers. No identifiers deleted beyond the two renamed components (no state removed — the deletion-sweep setter-variant rule is vacuously satisfied); new identifiers pairing-counted: CollectionPicker `loadError` 3/`setLoadError` 3/`saveError` 3/`setSaveError` 5; StatusLabelsModal `loadError` 3/`setLoadError` 3/`saveError` 3/`setSaveError` 4/`loadLabels` 3/`prevValue` 2; CollectionDetail `loadMoreError` 4/`setLoadMoreError` 8/`reorderError` 3/`setReorderError` 4; CollectionsTab `reorderError` 3/`setReorderError` 4. `py_compile` clean on `backend/main.py`. Design-lint (deno): **every strict category 0/pass; raw-`<button>` 125, delta 0** vs baseline (all new affordances use shared `Button`; the five ReadingStatusCard rows in the report became five AcquireCard rows; `text-teal-400` persists in TruncatedText by design until the C-pass — expected, not a regression). Unmount safety reasoned per converted site: every new setState lives in the same component as its async call, no timers or subscriptions added, and React 18 no-ops state sets after unmount; StatusLabelsModal stays mounted across open/close (`isOpen` gate), so both error states are explicitly reset on reopen.
- **Microcopy-pass flags (verbatim, new):** **"Couldn't load your collections."** · **"Created the collection, but couldn't add this title to it. Tap it below to try again."** · **"Couldn't load your labels."** · **"Couldn't load more titles."** · **"Couldn't save the new order. Try again?"** — reused approved strings: "Couldn't save your changes. Try again?" (toggle + persistLabel), "Try again" buttons.
- **Doc references for the C-pass (reported, not edited):** `ReadingStatusCard` still named in `docs/FRONTEND_AUDIT_S12.md` + historical CHANGELOG/ROADMAP entries (history stays); `CollapsibleSection` still named in `docs/DESIGN_SYSTEM.md` + `docs/FRONTEND_AUDIT_S12.md` + history. Observations for future ledgers, untouched: CollectionsTab :138/:173 and CollectionDetail :176/:596/:1058/:1241 call `localStorage` unguarded (siblings wrap theirs); FormField's `error` text renders without a live-region role (this session's field-level error adds its own `role="alert"` paragraph instead — rides beside the known settings-banner `role="alert"` gap in the B1 ledger); CollectionDetail's and CollectionsTab's in-page 3-dot menus use a z-40 backdrop that ties BottomNav's z-40 (nav stays tappable above the backdrop on mobile — spatially harmless today, same family as the v0.65.0 finding).
